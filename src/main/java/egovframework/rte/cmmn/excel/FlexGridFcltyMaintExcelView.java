 ...
 *   수정일 		 수정자		 수정내용
 *  -------		--------	---------------------------
 *  2016. 1. 18.		Jongmin		최초 생성
 *
 * Copyright (C) 2013 by LFIT  All right reserved.
 * </pre>
 */

public class FlexGridFcltyMaintExcelView extends AbstractExcelView {
...
//		setText(cell, "조회 결과 ");
		...
		String mntnFcltsCnstInfo = "";
		String tempMntnFcltsCnstInfo = null;
		int cellPos = 0;
		for (i = 0; i < resultList.size(); i++) {
			Map<String, String> td = (Map<String, String>) resultList.get(i);
			tempMntnFcltsCnstInfo = td.get("mntnFcltsCnstInfo");
			if(mntnFcltsCnstInfo.equals(tempMntnFcltsCnstInfo)) {
				for(j=0; j < headerList.size(); j++) {
					Map<String, String> head = (Map<String, String>) headerList.get(j);
					Object obj= td.get(head.get("abbr"));
					cell = getCell(sheet, 2+cellPos, j);
				...
				mntnFcltsCnstInfo = tempMntnFcltsCnstInfo;
				cell = getCell(sheet, 2+cellPos, 0);
				setText(cell, mntnFcltsCnstInfo);
				cellPos++;
				for(j=0; j < headerList.size(); j++) {
					Map<String, String> head = (Map<String, String>) headerList.get(j);
					Object obj= td.get(head.get("abbr"));
