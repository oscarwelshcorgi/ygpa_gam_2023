...
public class GamInfoCommFcltySpecMngController {
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@RequestMapping(value="/fclty/gamSelectInfoCommFcltySpecMngMntnRprDtlsList.do", method=RequestMethod.POST)
	@ResponseBody Map selectInfoCommFcltySpecMngMntnRprDtlsList(GamMntnRprDtlsVO searchVO) throws Exception {
		...
		List resultList = gamInfoCommFcltySpecMngService.selectInfoCommFcltySpecMngMntnRprDtlsList(searchVO);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/fclty/gamUpdateInfoCommFcltySpecMngAtchFileDirChage.do")
	@ResponseBody Map gamUpdateInfoCommFcltySpecMngAtchFileDirChage(GamAtchFileDirMngVO gamAtchFileDirMngVO) throws Exception {
		...
		if(!"I".equals(gamAtchFileDirMngVO.getDirFcltsJobSe())){
			map.put("resultCode", 1);
			map.put("resultMsg","다른 시설담당자가 생성한 디렉토리입니다. (이동 불가능)");
			return map;
		}

		gamInfoCommFcltySpecMngService.updateInfoCommFcltySpecMngAtchFileDirChage(gamAtchFileDirMngVO);
