...
**/
%>
GamSocApplyModule.prototype.validateDuration = function(startDate, endDate, startTitle, endTitle, startIgnore, endIgnore, equals) {
	var result = false;
...
GamSocApplyModule.prototype.saveData = function() {
	...
	if(!this.validateDuration(this.$('#applDate').val(), this.$('#periodFr').val(),  
								'신청일자', '보전처리기간 시작일', true, true, false)) {
		return;
	}	
	if(!this.validateDuration(this.$('#periodFr').val(), this.$('#periodTo').val(),  
								'보전처리기간 시작일', '보전처리기간 종료일', true, false, true)) {
		return;
	}	
	if(this._cmd == 'insert') {
		this.insertData();
