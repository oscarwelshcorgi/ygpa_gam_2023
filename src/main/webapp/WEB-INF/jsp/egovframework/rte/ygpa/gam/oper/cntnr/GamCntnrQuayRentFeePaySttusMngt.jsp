...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.loadComplete = function(params) {

    // 테이블 설정 //
    this.$("#cntnrQuayRentFeePaySttusMngtList").flexigrid({
        module: this,
        url: '/oper/cntnr/gamSelectCntnrQuayRentFeePaySttusMngtList.do',
        dataType: 'json',
        colModel : [
...
//					{display:'요금종류', name:'chrgeKnd',width:55, sortable:false,align:'center'},
        ...
        preProcess: function(module,data) {
        	...
        	module.makeDivValues('#cntnrQuayRentFeePaySttusMngtListSum', data);
            */
            return data;
        }
    });
    ...
    */
    ...
    /* 연체내역 목록 */
    this.$("#cntnrQuayRentFeePaySttusArrrgList").flexigrid({
        module: this,
        url: '/oper/cntnr/selectCntnrQuayRentFeePaySttusMngtDlyList.do',
        dataType: 'json',
        colModel : [
        ...
        preProcess: function(module,data) {
    		if (data.resultCode == "0") {
    			module.makeDivValues('#cntnrQuayRentFeePaySttusMngtListForm',data.resultDlyInfo);	// 리스트 값을 채운다

    			module.makeDivValues('#cntnrQuayRentFeePaySttusMngtSum', data.resultSummary); // 결과값을 채운다.
 ...
 GamCntnrQuayRentFeePaySttusMngtModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'btnUpdatePayDtls':	// 납부 현황 새로 고침
            this.doAction('/oper/cntnr/updateCntnrQuayRentFeePaySttusMngtList.do', null, function(module, result) {

                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamCntnrQuayRentFeePaySttusMngtSearchForm');
                    module.$('#cntnrQuayRentFeePaySttusMngtList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnNticArrrgCancelPk':
        	var rows = this.$('#cntnrQuayRentFeePaySttusArrrgList').selectedRows();
			if(rows.length==0) {
        ...
        case 'btnNticIssuePrint':
        	var rows = this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRows();
        	if(rows.length==0) {
            ...
            this.printPayNotice('/oper/cntnr/printCntnrQuayRentFeePayNotice.do', row);
        	break;
        case 'btnNticIssuePrint2':
            ...
            this.printPayNotice('/oper/cntnr/printCntnrQuayRentFeePayNotice2.do', row);
        	break;
        case 'btnCntnrQuayRentFeePaySttusMngtListExcelDownload':	// 엑셀 다운로드
    		this.$('#cntnrQuayRentFeePaySttusMngtList').flexExcelDown('/oper/cntnr/selectCntnrQuayRentFeePaySttusMngtListExcel.do');
    		break;
    }
};

GamCntnrQuayRentFeePaySttusMngtModule.prototype.nticArrrgSingle = function() {
	var rows = this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRows();
	if(rows.length==0) {
		alert('연체 고지 할 항목을 선택 하세요.');
		return;
...
//	}
	...
	if(this.resultDetail.arrrgNo!=null) {	// 기 연체 고지된 건이 있는지
	 	this.doAction('/oper/cntnr/updateNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/cntnr/printCntnrQuayRentFeePayNotice.do', row);
				}
			} else {
				alert(result.resultMsg);
			}
	 	...
	 	this.doAction('/oper/cntnr/insertNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/cntnr/printCntnrQuayRentFeePayNotice.do', row);
				}
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.nticArrrgCancelAll = function() {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/cntnr/cancelNticArrrg.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.nticArrrgCancelPk = function(ntic) {
	var row=this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRows()[0];
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/cntnr/cancelNticArrrgPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.nticArrrgPrintCancelPk = function(ntic) {
	var arrrgDetail = [
	               { name: 'prtAtCode', value: ntic.prtAtCode},
	               { name: 'fiscalYr', value: ntic.fiscalYr },
	               { name: 'billNo', value: ntic.billNo },
 	...
 	this.doAction('/oper/cntnr/cancelNticArrrgPrintPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCntnrQuayRentFeePaySttusMngtSearchForm');
    this.$("#cntnrQuayRentFeePaySttusMngtListTab").tabs("option", {active: 0});    // 탭을 전환 한다.
    this.$('#cntnrQuayRentFeePaySttusMngtList').flexOptions({params:searchOpt}).flexReload();
};

GamCntnrQuayRentFeePaySttusMngtModule.prototype.loadDetailPage = function() {
	var row = this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRows()[0];
	...
	var nticDetail = [
	 ...
	 this.doAction('/oper/cntnr/selectCntnrQuayRentFeePaySttusMngtDetail.do', nticDetail, function(module, result) {
		if (result.resultCode == "0") {
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.calcDlyDueDate = function() {
	// 납부기한 계산
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.loadArrrgPage = function() {
	var row = this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRows()[0];
	var searchOpt = [];
    ...
    this.$('#cntnrQuayRentFeePaySttusArrrgList').flexOptions({params:searchOpt}).flexReload();
	/*
	var dlyList = [
	               { name: 'prtAtCode', value: row.prtAtCode},
	               { name: 'chrgeKnd', value: row.chrgeKnd },
	...
	this.doAction('/oper/cntnr/selectCntnrQuayRentFeePaySttusMngtDlyList.do', dlyList, function(module, result) {
		if (result.resultCode == "0") {
			module.makeMultiDivValues('#cntnrQuayRentFeePaySttusMngtListForm',result.resultList , function(row) {
			} );	// 리스트 값을 채운다

			module.makeDivValues('#cntnrQuayRentFeePaySttusMngtSum', result.resultSummary); // 결과값을 채운다.
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.getMonthInterval = function (time1,time2,gubun){
	/* 날짜 계산 */
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.getDayInterval = function (fdate,tdate){
	var fd = Math.ceil(fdate.getTime()/(1000*60*60*24));
	var td = Math.ceil(tdate.getTime()/(1000*60*60*24));
  	return td-fd;
};

GamCntnrQuayRentFeePaySttusMngtModule.prototype.calculateArrrgFee = function() {
	 var fBasicRate = "" ;
	 var strText = "" ;
	// 연체료 계산
...
GamCntnrQuayRentFeePaySttusMngtModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	//change**
	if(newTabId=='tabs2' || newTabId=='tabs3') {
		if(this.$('#cntnrQuayRentFeePaySttusMngtList').selectedRowCount()!=1) {
...
<div class="window_main">
    ...
    <div class="emdPanel fillHeight">
        <div id="cntnrQuayRentFeePaySttusMngtListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange" data-onchange-before="onTabChangeBefore">
            <ul>
                <li><a href="#tabs1" class="emdTab">컨테이너부두임대사용료납부현황 목록</a></li>
                <li><a href="#tabs2" class="emdTab">컨테이너부두임대사용료납부현황 상세</a></li>
                <!-- change** -->
                <li><a href="#tabs3" class="emdTab">컨테이너부두임대사용료연체현황 목록</a></li>
            </ul>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                ...
                <div id="cntnrQuayRentFeePaySttusMngtListSum" class="emdControlPanel">
					<form id="summaryForm">
			...
			<div id="tabs3" class="emdTabPage" style="overflow: scroll;">
                ...
                <!-- <div class="emdControlPanel"><button id="btnSaveItem">저장</button><button id="btnCancelItem">취소</button><button id="btnRemoveItem">삭제</button></div>  -->
                    <form id="cntnrQuayRentFeePaySttusMngtListForm">
                    	<div>
                    ...
                    <h2>연체 고지 목록</h2>
                    <table id="cntnrQuayRentFeePaySttusArrrgList" style="display:none" class="fillHeight"></table>
                    <table style="width:100%; margin-bottom: 2px" id="cntnrQuayRentFeePaySttusMngtSum" class="summaryPanel">
						<tr>
