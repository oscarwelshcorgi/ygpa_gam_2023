...
<script>
		...
		this
				...
				.on(
						...
						function(event, module, row, grid, param) {
							// 이벤트내에선 모듈에 대해 선택한다.
							module.$("#fcltyMngtListTab").tabs("option", {
								active : 1
							}); // 탭을 전환 한다.
							...
							var detailInput = {
								gisAssetsCd : row["gisAssetsCd"],
								gisPrtFcltySeq : row["gisPrtFcltySeq"],
								gisAssetsPrtAtCode : row["gisAssetsPrtAtCode"],
								gisAssetsSubCd : row["gisAssetsSubCd"],
								gisPrtFcltyCd : module.$("#gisPrtFcltyCd")
										.val()
							};
							module
									.doAction(
											'<c:url value="/fclty/gamFcltyMngSelectView.do" />',
											detailInput,
											function(module, result) {
												...
												module
														.$("#gisAssetsCd")
														.val(
																result.detail.gisAssetsCd);
												module
														.$(
																"#gisAssetsPrtAtCode")
														.val(
																result.detail.gisAssetsPrtAtCode);
												module
														.$("#gisAssetsSubCd")
														.val(
																result.detail.gisAssetsSubCd);
												module
														.$("#gisPrtFcltySeq")
														.val(
																result.detail.gisPrtFcltySeq);
												module
														.$("#prtFcltyNm")
														.val(
																result.detail.prtFcltyNm);
												module
														.$("#prtFcltyStndrd")
														.val(
																result.detail.prtFcltyStndrd);
												module
														.$("#prtFcltyUnit")
														.val(
																result.detail.prtFcltyUnit);
												module
														.$("#prtFcltyInstlDt")
														.val(
																result.detail.prtFcltyInstlDt);
												module
														.$("#prtFcltyChangeDt")
														.val(
																result.detail.prtFcltyChangeDt);
												module
														.$("#prtFcltySe")
														.val(
																result.detail.prtFcltySe);
												module
														.$(
																"#prtFcltyMngEntrpsCd")
														.val(
																result.detail.prtFcltyMngEntrpsCd);
												module
														.$("#prtFcltyGisCd")
														.val(
																result.detail.prtFcltyGisCd);
												module.$("#registDt").val(
														result.detail.registDt);
											});
						});
	};
	...
	GamFcltyMngtModule.prototype.onButtonClick = function(buttonId) {
		...
		case "searchBtn":
			...
			this.$("#fcltyMngtList").flexOptions({
				params : searchOpt
			}).flexReload();
			break;
		...
		// 목록
		case "listBtn":
			this.$("#fcltyMngtListTab").tabs("option", {
				active : 0
			});
			break;
		...
		case "gisCodePopupBtn":
			this.doExecuteDialog("searchGisCodePopup", "업무사용자 암호변경",
					'<c:url value="/cmmn/popup/gamSearchGisCdPopupView.do"/>',
					{
						emplyrId : this.$("#emplyrId").val()
					});
			break;
		...
		case "saveBtn":
			...
			this.$("#prtFcltyInstlDt").val(
					this.$("#prtFcltyInstlDt").val().replace(/\-/g, ""));
			this.$("#prtFcltyChangeDt").val(
					this.$("#prtFcltyChangeDt").val().replace(/\-/g, ""));
			alert(this.$("#gisPrtFcltyCd").val());
			var inputVO = this.makeFormArgs("#fcltyManageVO");
			if (this.$("#cmd").val() == "insert") {

				this.doAction('<c:url value="/fclty/gamFcltyInsert.do" />',
						inputVO, function(module, result) {
							if (result.resultCode == "0") {
								var searchOpt = module
										.makeFormArgs("#fcltyForm");
								module.$("#fcltyMngtList").flexOptions({
									params : searchOpt
								}).flexReload();
								module.$("#fcltyMngtListTab").tabs("option", {
									active : 0
				...
				this.doAction('<c:url value="/fclty/gamFcltyUpdate.do" />',
						inputVO, function(module, result) {
							if (result.resultCode == "0") {
								var searchOpt = module
										.makeFormArgs("#fcltyForm");
		...
		case "deleteBtn":
			if (confirm("삭제하시겠습니까?")) {
				...
				var inputVO = this.makeFormArgs("#fcltyManageVO");
				this.doAction('<c:url value="/fclty/gamFcltyDelete.do" />',
						inputVO, function(module, result) {
							if (result.resultCode == "0") {
								var searchOpt = module
										.makeFormArgs("#fcltyForm");
...
<div class="window_main">
			...
			<form id="fcltyForm">
				...
				<table class="searchPanel">
					<tbody>
						<tr>
							<th>항만시설 명</th>
							<td><input id="searchKeyword" type="text" size="60"
	...
	<div class="emdPanel">
		<div id="fcltyMngtListTab" class="emdTabPanel"
			...
			<div id="tabs2" class="emdTabPage"
				...
				<form id="fcltyManageVO">
					<input type="hidden" id="cmd" /> <input type="hidden"
						id="gisPrtFcltySeq" /> <input type="hidden" id="gisPrtFcltyCd" />
					<table class="searchPanel">
						<tr>
							<th width="20%" height="23" class="required_text">GIS 자산 코드<img
								src="<c:url value='/images/egovframework/com/cmm/icon/required.gif' />"
								width="15" height="15" alt="필수입력표시" /></th>
							<td><input type="text" size="40" id="gisAssetsCd"
								disabled="disabled" />
								<button id="gisCodePopupBtn">자산코드 검색</button></td>
						</tr>
						<tr>
							<th width="20%" height="23" class="required_text">GIS 자산 SUB
								코드<img
								src="<c:url value='/images/egovframework/com/cmm/icon/required.gif' />"
								width="15" height="15" alt="필수입력표시" />
							</th>
							<td><input type="text" size="40" id="gisAssetsSubCd"
								disabled="disabled" /></td>
						</tr>
						<tr>
							<th width="20%" height="23" class="required_text">GIS 자산 항코드<img
								src="<c:url value='/images/egovframework/com/cmm/icon/required.gif' />"
								width="15" height="15" alt="필수입력표시" /></th>
							<td><input type="text" size="40" id="gisAssetsPrtAtCode"
								disabled="disabled" /></td>
						</tr>
						<tr>
							<th width="20%" height="23" class="required_text">항만시설 GIS
								코드</th>
						...
						<tr>
							<th width="20%" height="23" class="required_text">항만시설 관리 업체
								코드</th>
							<td><input type="text" size="40" id="prtFcltyMngEntrpsCd" /></td>
						</tr>
						<tr>
							<th width="20%" height="23" class="required_text">항만시설 설치일자</th>
							<td><input id="prtFcltyInstlDt" type="text" class="emdcal"
								size="20" title="설치일자" disabled="disabled" /></td>
						</tr>
						<tr>
							<th width="20%" height="23" class="required_text">항만시설 변경일자</th>
							<td><input id="prtFcltyChangeDt" type="text" class="emdcal"
								size="20" title="변경일자" disabled="disabled" /></td>
						</tr>
						<tr id="displayDate">
							<th width="20%" height="23" class="required_text">등록일자</th>
							<td><input type="text" size="40" id="registDt"
								disabled="disabled" /></td>
						</tr>
					</table>
				</form>
