...
GamCntnrQuayNticArrvlDtaInqireModule.prototype.loadComplete = function() {
    ...
    // 컨테이너부두임대상세 테이블 설정
    this.$("#cntnrQuayNticArrvlDtaInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/cntnr/gamSelectCntnrQuayNticArrvlDtaInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#cntnrQuayNticArrvlDtaInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module.$('#gamCntnrQuayNticArrvlDtaInqireForm :input').val('');

        module.makeFormValues('#gamCntnrQuayNticArrvlDtaInqireForm', row);
        module._editData=module.getFormValues('#gamCntnrQuayNticArrvlDtaInqireForm', row);
        module._editRow=module.$('#cntnrQuayNticArrvlDtaInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamCntnrQuayNticArrvlDtaInqireForm');
        module.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#cntnrQuayNticArrvlDtaInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds()[0];
    });
    ...
    this.$("#cntnrQuayNticArrvlDtaInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cntnrQuayNticArrvlDtaInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds()[0];
...
GamCntnrQuayNticArrvlDtaInqireModule.prototype.onCalc = function() {
 ...
 GamCntnrQuayNticArrvlDtaInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            this.$("#cntnrQuayNticArrvlDtaInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamCntnrQuayNticArrvlDtaInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#cntnrQuayNticArrvlDtaInqireDetailList").flexRemove();
            this.$("#cntnrQuayNticArrvlDtaInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#cntnrQuayNticArrvlDtaInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/cntnr/gamInsertCntnrQuayNticArrvlDtaInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCntnrQuayNticArrvlDtaInqireSearchForm');
                            module.$('#cntnrQuayNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamCntnrQuayNticArrvlDtaInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamCntnrQuayNticArrvlDtaInqireForm', {_updtId:'I'});
                ...
                this.doAction('<c:url value="/oper/cntnr/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamCntnrQuayNticArrvlDtaInqireForm');
                        module.$('#cntnrQuayNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexReload();
                        module.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#cntnrQuayNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
            ...
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamCntnrQuayNticArrvlDtaInqireForm');
                    ...
                    this.$("#cntnrQuayNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamCntnrQuayNticArrvlDtaInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#cntnrQuayNticArrvlDtaInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("컨테이너부두임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexGetRow(this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexRemoveRow(this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#cntnrQuayNticArrvlDtaInqireDetailList').flexAddRow(this._editData);
            }
...
GamCntnrQuayNticArrvlDtaInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    ...
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#cntnrQuayNticArrvlDtaInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#cntnrQuayNticArrvlDtaInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
        ...
        <div id="cntnrQuayNticArrvlDtaInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <!--<div class="emdControlPanel"></div>-->
                    <form id="gamCntnrQuayNticArrvlDtaInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="cntnrQuayNticArrvlDtaInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="cntnrQuayNticArrvlDtaInqireDetailList" style="display:none"></table>
