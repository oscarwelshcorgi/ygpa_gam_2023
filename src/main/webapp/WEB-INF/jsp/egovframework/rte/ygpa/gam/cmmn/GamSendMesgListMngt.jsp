...
GamSendMesgListMngtModule.prototype.loadComplete = function() {
    ...
    this.$("#prtFcltyUseExprInqireList").flexigrid({
        ...
        dataType: 'json',
        colModel : [
                    {display:'SMS순번', name:'prtAtCodeNm',width:60, sortable:false,align:'center'},
                    {display:'수신번호', name:'rentMngNo',width:100, sortable:false,align:'center'},
                    {display:'회신번호', name:'entrpsNm',width:170, sortable:false,align:'center'},
                    {display:'내용', name:'entrpscd',width:90, sortable:false,align:'center'},
                    {display:'전송일자', name:'grUsagePdFrom',width:100, sortable:false,align:'center'},
                    {display:'전송상태', name:'grUsagePdTo',width:100, sortable:false,align:'center'},
                    {display:'관리년도', name:'reqstSeCdNm',width:60, sortable:false,align:'center'},
                    {display:'관리번호', name:'prmisnYn',width:60, sortable:false,align:'center'},
                    {display:'관리횟수', name:'sanctnSttus',width:60, sortable:false,align:'center'},
                    {display:'등록자', name:'grFee',width:120, sortable:false,align:'center', displayFormat: 'number'}
                    ],
    ...
    this.$("#prtFcltyUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module.$('#GamSendMesgListMngtForm :input').val('');

        module.makeFormValues('#GamSendMesgListMngtForm', row);
        module._editData=module.getFormValues('#GamSendMesgListMngtForm', row);
        ...
        var searchOpt=module.makeFormArgs('#GamSendMesgListMngtForm');
        module.$('#prtFcltyUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#prtFcltyUseExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#prtFcltyUseExprInqireDetailList').selectedRowIds()[0];
    });
    ...
    this.$("#prtFcltyUseExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
...
GamSendMesgListMngtModule.prototype.onCalc = function() {
 ...
 GamSendMesgListMngtModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            ...
            this.$('#GamSendMesgListMngtForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#prtFcltyUseExprInqireDetailList").flexRemove();
            this.$("#prtFcltyUseExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/gnrl/gamInsertPrtFcltyUseExprInqireRenew.do" />', rows[0], function(module, result) {
                        ...
                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#GamSendMesgListMngtSearchForm');
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#prtFcltyUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#prtFcltyUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#GamSendMesgListMngtForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#GamSendMesgListMngtForm', {_updtId:'I'});
                ...
                this.doAction('<c:url value="/oper/gnrl/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#GamSendMesgListMngtForm');
                        module.$('#prtFcltyUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#prtFcltyUseExprInqireDetailList').flexReload();
                        module.$('#prtFcltyUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
            ...
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#GamSendMesgListMngtForm');
                    ...
                    this.$("#prtFcltyUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#GamSendMesgListMngtForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this._editRow=this.$('#prtFcltyUseExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#prtFcltyUseExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("항만시설사용상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#prtFcltyUseExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#prtFcltyUseExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#prtFcltyUseExprInqireDetailList').flexGetRow(this.$('#prtFcltyUseExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#prtFcltyUseExprInqireDetailList').flexRemoveRow(this.$('#prtFcltyUseExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#prtFcltyUseExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#prtFcltyUseExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamSendMesgListMngtModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        ...
        break;
    case 'tabs3':
        var row = this.$('#prtFcltyUseExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamSendMesgListMngtModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
                ...
                <table style="width:100%;" class="searchPanel">
                    <tbody>
                        <tr>
                            ...
                            <th>관리년도</th>
                            <td>
                                <input id="sEntrpscd" type="text" size="4">
                            </td>
                            <th>관리번호</th>
                            <td>
                            	<input id="sEntrpscd" type="text" size="3">
                            </td>
                            <th>관리 횟수</th>
                            <td>
                            	<input id="sEntrpscd" type="text" size="2">
                            </td>
                        ...
                        <tr>
                            <th>전송상태</th>
                            <td colspan="9">
                                	전송중 <input id="" type="checkbox">
                                	전송완료 <input id="" type="checkbox">
                                	전송실패 <input id="" type="checkbox">
                                	전송취소 <input id="" type="checkbox">
                            </td>                         
                        </tr>
                    </tbody>
                </table>
                    ...
                    <table style="width:100%;">
                        <tr>
                            <td style="text-align: right">
                                <button id="addAssetRentRenew">재전송</button>
                            </td>
                        </tr>
                    </table>
                </div>
