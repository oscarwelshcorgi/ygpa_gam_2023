...
<%
  /**
  * @Class Name : GamTrainPortUseExprInqire.jsp
 ...
 */
function GamTrainPortUseExprInqireModule() {}

GamTrainPortUseExprInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamTrainPortUseExprInqireModule.prototype.loadComplete = function() {

    // 철송장임대 테이블 설정
    this.$("#trainPortUseExprInqireList").flexigrid({
        module: this,
        url: '<c:url value="/oper/train/gamSelectTrainPortUseExprInqireList.do" />',
        dataType: 'json',
        colModel : [
    ...
    // 철송장임대상세 테이블 설정
    this.$("#trainPortUseExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/train/gamSelectTrainPortUseExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#trainPortUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        module.$('#cmd').val('modify');

        module.$('#gamTrainPortUseExprInqireForm :input').val('');

        module.makeFormValues('#gamTrainPortUseExprInqireForm', row);
        module._editData=module.getFormValues('#gamTrainPortUseExprInqireForm', row);
        module._editRow=module.$('#trainPortUseExprInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamTrainPortUseExprInqireForm');
        module.$('#trainPortUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#trainPortUseExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#trainPortUseExprInqireDetailList').selectedRowIds()[0];
    });

    this.$("#trainPortUseExprInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#trainPortUseExprInqireListTab").tabs("option", {active: 1});
        module.$('#cmd').val('modify');
        module.$('#gamTrainPortUseExprInqireForm :input').val('');
    ...
    this.$("#trainPortUseExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#trainPortUseExprInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#trainPortUseExprInqireDetailList').selectedRowIds()[0];
...
GamTrainPortUseExprInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamTrainPortUseExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            this.$("#trainPortUseExprInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamTrainPortUseExprInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#trainPortUseExprInqireDetailList").flexRemove();
            this.$("#trainPortUseExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#trainPortUseExprInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/train/gamInsertTrainPortUseExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamTrainPortUseExprInqireSearchForm');
                            module.$('#trainPortUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#trainPortUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#trainPortUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamTrainPortUseExprInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamTrainPortUseExprInqireForm', {_updtId:'I'});
                ...
                this.doAction('<c:url value="/oper/train/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamTrainPortUseExprInqireForm');
                        module.$('#trainPortUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#trainPortUseExprInqireDetailList').flexReload();
                        module.$('#trainPortUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#trainPortUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
            ...
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamTrainPortUseExprInqireForm');
                    ...
                    this.$("#trainPortUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamTrainPortUseExprInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#trainPortUseExprInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#trainPortUseExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#trainPortUseExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("철송장임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#trainPortUseExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#trainPortUseExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#trainPortUseExprInqireDetailList').flexGetRow(this.$('#trainPortUseExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#trainPortUseExprInqireDetailList').flexRemoveRow(this.$('#trainPortUseExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#trainPortUseExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#trainPortUseExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamTrainPortUseExprInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamTrainPortUseExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamTrainPortUseExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#trainPortUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamTrainPortUseExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#trainPortUseExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#trainPortUseExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamTrainPortUseExprInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamTrainPortUseExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamTrainPortUseExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="trainPortUseExprInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">철송장임대(만기도래) 목록</a></li>
                <li><a href="#tabs2" class="emdTab">철송장임대(만기도래) 내역</a></li>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="trainPortUseExprInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <div class="emdControlPanel"></div>
                    <form id="gamTrainPortUseExprInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="trainPortUseExprInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="trainPortUseExprInqireDetailList" style="display:none"></table>
