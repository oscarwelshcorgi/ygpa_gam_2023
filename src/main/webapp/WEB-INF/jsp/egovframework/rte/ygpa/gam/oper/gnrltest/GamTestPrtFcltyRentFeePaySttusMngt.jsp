...
<%
/**
 * @Class Name : GamTestPrtFcltyRentFeePaySttusMngt.jsp
 ...
 */
function GamTestPrtFcltyRentFeePaySttusMngtModule() {}

GamTestPrtFcltyRentFeePaySttusMngtModule.prototype = new EmdModule(1040, 620);

// 페이지가 호출 되었을때 호출 되는 함수
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.loadComplete = function(params) {

    // 테이블 설정 //
    this.$("#prtFcltyRentFeePaySttusMngtList").flexigrid({
        module: this,
        url: '/oper/gnrltest/gamSelectPrtFcltyRentFeePaySttusMngtList.do',
        dataType: 'json',
        colModel : [
...
//					{display:'요금종류', name:'chrgeKnd',width:55, sortable:false,align:'center'},
    ...
    */
    ...
    this.$("#prtFcltyRentFeePaySttusArrrgList").flexigrid({
        module: this,
        url: '/oper/gnrltest/selectPrtFcltyRentFeePaySttusMngtDlyList.do',
        dataType: 'json',
        colModel : [
 ...
 * 정의 된 버튼 클릭 시
 */
 GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.onButtonClick = function(buttonId) {
     var opts=null;
    ...
    switch(buttonId) {
        ...
        case 'popupEntrpsInfo':
            var searchOpt=this.makeFormArgs('#gamTestPrtFcltyRentFeePaySttusMngtSearchForm');
        ...
        case 'btnUpdatePayDtls':	// 납부 현황 새로 고침
            this.doAction('/oper/gnrltest/updatePrtFcltyRentFeePaySttusMngtList.do', null, function(module, result) {

                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamTestPrtFcltyRentFeePaySttusMngtSearchForm');
        ...
        case 'btnRecivePay':
        	...
        	break;
        case 'btnNticArrrg':
            this.doExecuteDialog('nticArrrgPopup', '연체 일괄 고지', '/oper/gnrltest/showNticArrrgPopup.do', opts);
        	break;
        ...
        case 'btnNticIssuePrint':
            ...
            this.printPayNotice('/oper/gnrltest/printPrtFcltyRentFeePayNotice.do', row);
        	break;
        case 'btnNticIssuePrint2':
            ...
            this.printPayNotice('/oper/gnrltest/printPrtFcltyRentFeePayNotice2.do', row);
        	break;
        case 'btnPrtFcltyRentFeePaySttusMngtListExcelDownload':	// 엑셀 다운로드
    		this.$('#prtFcltyRentFeePaySttusMngtList').flexExcelDown('/oper/gnrltest/selectPrtFcltyRentFeePaySttusMngtListExcel.do');
    		break;
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.nticArrrgSingle = function() {
	var rows = this.$('#prtFcltyRentFeePaySttusMngtList').selectedRows();
	if(rows.length==0) {
		alert('연체 고지 할 항목을 선택 하세요.');
		return;
...
//	}
	...
	if(this.resultDetail.arrrgNo!=null) {	// 기 연체 고지된 건이 있는지
	 	this.doAction('/oper/gnrltest/updateNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/gnrltest/printPrtFcltyRentFeePayNotice.do', row);
				}
			} else {
				alert(result.resultMsg);
			}
	 	...
	 	this.doAction('/oper/gnrltest/insertNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/gnrltest/printPrtFcltyRentFeePayNotice.do', row);
				}
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.receiveFeeSingle = function() {
    if(this.$('#prtFcltyRentFeePaySttusMngtList').selectedRowCount()>0) {

        var rows = this.$('#prtFcltyRentFeePaySttusMngtList').selectedRows()[0];

     	this.doAction('/oper/gnrltest/checkOcrResult.do', rows, function(module, result) {
    		if (result.resultCode == "0") {
    	    	...
    	    	module.doExecuteDialog('feePayPopup', '수납 처리', '/oper/gnrltest/showFeePayPopup.do', opts);
    		} else {
    			alert(result.resultMsg);
    		}
    	});
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.nticArrrgCancelAll = function() {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/gnrltest/cancelNticArrrg.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.nticArrrgCancelPk = function(ntic) {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/gnrltest/cancelNticArrrgPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.nticArrrgPrintCancelPk = function(ntic) {
	var arrrgDetail = [
	               { name: 'prtAtCode', value: ntic.prtAtCode},
	               { name: 'fiscalYr', value: ntic.fiscalYr },
	               { name: 'billNo', value: ntic.billNo },
 	...
 	this.doAction('/oper/gnrltest/cancelNticArrrgPrintPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamTestPrtFcltyRentFeePaySttusMngtSearchForm');
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.loadDetailPage = function() {
	var row = this.$('#prtFcltyRentFeePaySttusMngtList').selectedRows()[0];
	...
	var nticDetail = [
	 ...
	 this.doAction('/oper/gnrltest/selectPrtFcltyRentFeePaySttusMngtDetail.do', nticDetail, function(module, result) {
		if (result.resultCode == "0") {
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.calcDlyDueDate = function() {
	// 납부기한 계산
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.loadArrrgPage = function() {
	var row = this.$('#prtFcltyRentFeePaySttusMngtList').selectedRows()[0];
	var searchOpt = [];
	...
	this.doAction('/oper/gnrltest/selectPrtFcltyRentFeePaySttusMngtDlyList.do', dlyList, function(module, result) {
		if (result.resultCode == "0") {
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.getMonthInterval = function (time1,time2,gubun){
	/* 날짜 계산 */
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.getDayInterval = function (fdate,tdate){
	var fd = Math.ceil(fdate.getTime()/(1000*60*60*24));
	var td = Math.ceil(tdate.getTime()/(1000*60*60*24));
  	return td-fd;
};

GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.calculateArrrgFee = function() {
	 var fBasicRate = "" ;
	 var strText = "" ;
	// 연체료 계산
 ...
 //alert("계산61==>"+iTermMonth+"::"+iTermDay+"::"+fBasicRate+"::"+strText);
 	...
 	//iDlyAmnt = Math.floor(parseInt(this.resultArrrg.fee * fBasicRate * iTermDay / 365));	// 연체료는 원단위 절삭 안함 (허정석 차장님 요청)
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	//change**
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
...
GamTestPrtFcltyRentFeePaySttusMngtModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
    ...
    case 'feePayPopup':
    	if (msg != 'cancel') {
            ...
            this.doAction('/oper/gnrltest/updateRevCollRcvdTp.do', arg, function(module, result) {
...
var module_instance = new GamTestPrtFcltyRentFeePaySttusMngtModule();
</script>
<!-- 아래는 고정 -->
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamTestPrtFcltyRentFeePaySttusMngtSearchForm">
