...
GamHtldNticArrvlDtaInqireModule.prototype.loadRentList = function() {
...
GamHtldNticArrvlDtaInqireModule.prototype.loadDetailForm = function() {
 	var row = this.$('#htldNticArrvlDtaInqireList').selectedRows()[0];
	...
	this.makeDivValues('#gamHtldNticArrvlDtaInqireForm', row); // 결과값을 채운다.

	this.doAction('<c:url value="/oper/htld/gamSelectHtldNticArrvlDtaInqireDetailList.do" />', detailParam, function(module, result) {

		if (result.resultCode == "0") {
			
			module.makeMultiDivValues('#htldNticArrvlDtaInqireDetailForm',result.resultList , function(row) {
			} );	// 리스트 값을 채운다
		} else {
 ...
 GamHtldNticArrvlDtaInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'openRentFee':	// 사용료 관리 화면을 호출 한다.
	        if(this.$('#htldNticArrvlDtaInqireList').selectedRowCount()==1) {
	         	var row = this.$('#htldNticArrvlDtaInqireList').selectedRows()[0];
...
GamHtldNticArrvlDtaInqireModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	if(oldTabId=="tabs1" && this.$('#htldNticArrvlDtaInqireList').selectedRowCount()==0) {
