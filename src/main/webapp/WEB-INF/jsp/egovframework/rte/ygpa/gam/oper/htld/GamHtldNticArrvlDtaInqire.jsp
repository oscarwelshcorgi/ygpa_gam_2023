...
<%
  /**
  * @Class Name : GamHtldNticArrvlDtaInqire.jsp
 ...
 */
function GamHtldNticArrvlDtaInqireModule() {}

GamHtldNticArrvlDtaInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamHtldNticArrvlDtaInqireModule.prototype.loadComplete = function() {

    // 배후단지임대 테이블 설정
    this.$("#htldNticArrvlDtaInqireList").flexigrid({
        module: this,
        url: '<c:url value="/oper/htld/gamSelectHtldNticArrvlDtaInqireList.do" />',
        dataType: 'json',
        colModel : [
					{display:'항코드', name:'prtAtCode',width:40, sortable:false,align:'center'},
					{display:'항코드', name:'prtAtCodeNm',width:55, sortable:false,align:'center'},
    ...
    // 배후단지임대상세 테이블 설정
    this.$("#htldNticArrvlDtaInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/htld/gamSelectHtldNticArrvlDtaInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#htldNticArrvlDtaInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        module.$('#cmd').val('modify');

        module.$('#gamHtldNticArrvlDtaInqireForm :input').val('');

        module.makeFormValues('#gamHtldNticArrvlDtaInqireForm', row);
        module._editData=module.getFormValues('#gamHtldNticArrvlDtaInqireForm', row);
        module._editRow=module.$('#htldNticArrvlDtaInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamHtldNticArrvlDtaInqireForm');
        module.$('#htldNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#htldNticArrvlDtaInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds()[0];
    });

    this.$("#htldNticArrvlDtaInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 1});
        module.$('#cmd').val('modify');
        module.$('#gamHtldNticArrvlDtaInqireForm :input').val('');
    ...
    this.$("#htldNticArrvlDtaInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds()[0];
...
GamHtldNticArrvlDtaInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamHtldNticArrvlDtaInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'searchBtn':
            ...
            var searchOpt=this.makeFormArgs('#gamHtldNticArrvlDtaInqireSearchForm');
            this.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 0});
            this.$('#htldNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();

            break;

        // 최초신청
        case 'addAssetRentFirst':
            this.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamHtldNticArrvlDtaInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#htldNticArrvlDtaInqireDetailList").flexRemove();
            this.$("#htldNticArrvlDtaInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#htldNticArrvlDtaInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/htld/gamInsertHtldNticArrvlDtaInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamHtldNticArrvlDtaInqireSearchForm');
                            module.$('#htldNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#htldNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#htldNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamHtldNticArrvlDtaInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamHtldNticArrvlDtaInqireForm', {_updtId:'I'});
                ...
                this.doAction('<c:url value="/oper/htld/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamHtldNticArrvlDtaInqireForm');
                        module.$('#htldNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#htldNticArrvlDtaInqireDetailList').flexReload();
                        module.$('#htldNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
            ...
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamHtldNticArrvlDtaInqireForm');
                    ...
                    this.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamHtldNticArrvlDtaInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#htldNticArrvlDtaInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#htldNticArrvlDtaInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#htldNticArrvlDtaInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("배후단지임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#htldNticArrvlDtaInqireDetailList').flexGetRow(this.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#htldNticArrvlDtaInqireDetailList').flexRemoveRow(this.$('#htldNticArrvlDtaInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#htldNticArrvlDtaInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#htldNticArrvlDtaInqireDetailList').flexAddRow(this._editData);
            }
...
GamHtldNticArrvlDtaInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamHtldNticArrvlDtaInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamHtldNticArrvlDtaInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#htldNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamHtldNticArrvlDtaInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#htldNticArrvlDtaInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#htldNticArrvlDtaInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamHtldNticArrvlDtaInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamHtldNticArrvlDtaInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamHtldNticArrvlDtaInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="htldNticArrvlDtaInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">배후단지임대(고지도래) 목록</a></li>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="htldNticArrvlDtaInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <!-- <div class="emdControlPanel"></div> -->
                    <form id="gamHtldNticArrvlDtaInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="htldNticArrvlDtaInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="htldNticArrvlDtaInqireDetailList" style="display:none"></table>
