...
<%
  /**
  * @Class Name : GamCmmnCntrShedRentFeeMngt.jsp
 ...
 */
function GamCmmnCntrShedRentFeeMngtModule() {}

GamCmmnCntrShedRentFeeMngtModule.prototype = new EmdModule(1000, 550);

// 페이지가 호출 되었을때 호출 되는 함수
GamCmmnCntrShedRentFeeMngtModule.prototype.loadComplete = function() {

    // 테이블 설정 //       
    this.$("#operResultList").flexigrid({
        module: this,
        url: '<c:url value="/oper/shed/gamSelectCmmnCntrShedRentFeeMngtList.do" />',
        dataType: 'json',
        colModel : [
 ...
 * 정의 된 버튼 클릭 시
 */
 GamCmmnCntrShedRentFeeMngtModule.prototype.onButtonClick = function(buttonId) {

    switch(buttonId) {

        // 조회
        case 'searchBtn':
            var searchOpt=this.makeFormArgs('#gamCmmnCntrShedRentFeeMngtSearchForm');
        ...
        case 'btnSaveItem':
        	if( this.$('#prtAtCode').val() == '' ) {
	        	...
	        	var inputVO=this.makeFormArgs('#gamCmmnCntrShedRentFeeMngtForm');
	        	
	        	this.doAction('<c:url value="/oper/shed/gamUpdateCmmnCntrShedRentFeeMngt.do" />', inputVO, function(module, result) {
	
	                if(result.resultCode=='0') {
	                    var searchOpt=module.makeFormArgs('#gamCmmnCntrShedRentFeeMngtSearchForm');
         ...
         // 취소
        case 'btnCancelItem':
            this.$('#gamCmmnCntrShedRentFeeMngtForm :input').val("");
            break;
        ...
        case 'saveNticListBtn':
            ...
            if(reglist.length > 0){
                ...
                this.doAction('<c:url value="/oper/shed/gamInsertCmmnCntrShedRentFeeMngtNtic.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt = module.makeFormArgs("#gamCmmnCntrShedRentFeeMngtSearchForm");
                        module.$("#operResultList").flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        case 'cancelNticListBtn': 
            ...
            if(reglist.length > 0){
                ...
                var inputVO = {nticCnts: nticCnts, prtAtCodes: prtAtCodes, mngYears: mngYears, mngNos: mngNos, mngCnts: mngCnts, nticnos: nticnos, accnutYears: accnutYears };
                this.doAction('<c:url value="/oper/shed/gamDeleteCmmnCntrShedRentFeeMngtNtic.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt = module.makeFormArgs("#gamCmmnCntrShedRentFeeMngtSearchForm");
                        module.$("#operResultList").flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        case 'saveNticDetailBtn':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.doAction('<c:url value="/oper/shed/gamInsertCmmnCntrShedRentFeeMngtNticSingle.do" />', inputVO, function(module, result) {
        ...
        case 'cancelNticDetailBtn':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.doAction('<c:url value="/oper/shed/gamDeleteCmmnCntrShedRentFeeMngtNticSingle.do" />', inputVO, function(module, result) {
...
GamCmmnCntrShedRentFeeMngtModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamCmmnCntrShedRentFeeMngtModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCmmnCntrShedRentFeeMngtSearchForm');
    //this.showAlert(searchOpt);
    this.$('#operResultList').flexOptions({params:searchOpt}).flexReload();
};

GamCmmnCntrShedRentFeeMngtModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
...
GamCmmnCntrShedRentFeeMngtModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamCmmnCntrShedRentFeeMngtModule();
</script>
<!-- 아래는 고정 -->
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamCmmnCntrShedRentFeeMngtSearchForm">
    ...
    <div class="emdPanel">
        <div id="operResultListTab" class="emdTabPanel" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">공컨장치장임대료 목록내역</a></li>
                <li><a href="#tabs2" class="emdTab">공컨장치장임대료 상세조회내역</a></li>
            </ul>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                ...
                <div class="emdControlPanel">
                    ...
                    <form id="gamCmmnCntrShedRentFeeMngtForm">
