...
GamNticArrrgPopupModule.prototype.loadComplete = function(searchOpt) {
	...
	// 그리드 설정
    this.$("#trainPortRentFeeArrrgList").flexigrid({
        module: this,
        url: '<c:url value="/oper/train/selectNticArrrgList.do" />',
        dataType: 'json',
        colModel : [
	...
	this.$("#trainPortRentFeeArrrgList").on("onItemDoubleClick", function(event, module, row, grid, param) {
...
GamNticArrrgPopupModule.prototype.onButtonClick = function(buttonId) {
	...
	case 'btnSaveArrrg':	// 연체 일괄 고지
		if(confirm("조회된 건에 대해서 연체 일괄 고지를 하시겠습니까?")) {
	        ...
	        this.doAction('<c:url value="/oper/train/insertNticArrrgList.do" />', searchOpt, function(module, result) {
...
GamNticArrrgPopupModule.prototype.loadData = function() {
	var searchOpt=this.makeFormArgs("#gamNticArrrgListVO");
 	this.$("#trainPortRentFeeArrrgList").flexOptions({params:searchOpt}).flexReload();
};
...
<div class="dialog fillHeight">
	...
	<div class="emdPanel fillHeight">
		<table id="trainPortRentFeeArrrgList" style="display: none" class="fillHeight"></table>
