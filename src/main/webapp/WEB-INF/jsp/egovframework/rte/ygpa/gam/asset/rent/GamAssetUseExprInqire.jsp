...
GamAssetUseExprInqireModule.prototype = new EmdModule(1100, 650);
...
GamAssetUseExprInqireModule.prototype.loadComplete = function() {
    ...
    this.$("#assetUseExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/asset/rent/gamSelectAssetUseExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#assetUseExprInqireList').selectedRowIds()[0];
...
GamAssetUseExprInqireModule.prototype.onCalc = function() {
 ...
 GamAssetUseExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#assetUseExprInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/asset/rent/gamInsertAssetUseExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamAssetUseExprInqireSearchForm');
                            module.$('#assetUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
...
GamAssetUseExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    ...
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#assetUseExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
