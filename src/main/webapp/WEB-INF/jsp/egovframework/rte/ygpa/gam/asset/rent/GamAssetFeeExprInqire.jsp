...
<%
  /**
  * @Class Name : GamAssetFeeExprInqire.jsp
...
GamAssetFeeExprInqireModule.prototype.loadComplete = function() {
    ...
    // 자산임대상세 테이블 설정
    this.$("#assetFeeExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/asset/rent/gamSelectAssetFeeExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetFeeExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamAssetFeeExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            ...
            //this.$("#assetFeeExprInqireDetailList").flexRemove();
            this.$("#assetFeeExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#assetFeeExprInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/asset/rent/gamInsertAssetFeeExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamAssetFeeExprInqireSearchForm');
                            module.$('#assetFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#assetFeeExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };
                ...
                this.doAction('<c:url value="/asset/rent/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamAssetRentForm');
                        module.$('#assetFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#assetFeeExprInqireDetailList').flexReload();
                        module.$('#assetFeeExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this._editRow=this.$('#assetFeeExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#assetFeeExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("자산임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#assetFeeExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#assetFeeExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#assetFeeExprInqireDetailList').flexGetRow(this.$('#assetFeeExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#assetFeeExprInqireDetailList').flexRemoveRow(this.$('#assetFeeExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#assetFeeExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamAssetFeeExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamAssetFeeExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#assetFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamAssetFeeExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#assetFeeExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#assetFeeExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamAssetFeeExprInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamAssetFeeExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamAssetFeeExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="assetRentListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">자산임대(고지도래) 목록</a></li>
