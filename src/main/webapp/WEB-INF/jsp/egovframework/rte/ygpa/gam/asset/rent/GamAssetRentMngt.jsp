...
%>
    ...
    GamAssetRentMngtModule.prototype.loadComplete = function(param) {
        ...
        this.$("#assetRentMngtList").flexigrid({
            module: this,
            url: '/asset/rent/gamSelectAssetRentMngtList.do',
            dataType: 'json',
            colModel : [
...
//                        {display:'결재일시', name:'sanctnDt',width:110, sortable:false,align:'center'},
                       ...
                       /*  
                        ...
                        {display:'총사용시작일', name:'grUsagePdFrom',width:80, sortable:false,align:'center'},
                        {display:'총사용종료일', name:'grUsagePdTo',width:80, sortable:false,align:'center'},
                        {display:'총면적', name:'grAr',width:100, sortable:false,align:'right', displayFormat: 'number', displayOption:{format:"0,000.00"}},
                        {display:'총사용료', name:'grFee',width:100, sortable:false,align:'right', displayFormat: 'number', displayOption:"0,000"},
                        {display:'총감면사용료', name:'grRdcxptFee',width:100, sortable:false,align:'right', displayFormat: 'number'},
        ...
        this.$("#assetRentDetailList").flexigrid({
            module: this,
            url: '/asset/rent/gamSelectAssetRentMngtDetailList.do',
            dataType: 'json',
            colModel : [
        ...
        this.$("#assetRentFileList").flexigrid({
            module: this,
            url: '/asset/rent/gamSelectAssetRentMngtFileList.do',
            dataType: 'json',
            colModel : [
        ...
        this.$("#assetRentFileList").on('onItemSelected', function(event, module, row, grid, param) {
            ...
            if(row.filenmPhysicl!=null || row.filenmPhysicl!='') {
    			...
    			if(ext=='jpg' || ext=='jpeg' || ext=='bmp' || ext=='png' || ext=='gif') {
    			    $imgURL = module.getUrl("/asset/rent/getRentAttachFile.do?physicalFileNm=")+filenm;
    ...
    GamAssetRentMngtModule.prototype.setButtonState = function() {
    	...
    	case 0:
    		...
    		if(rownum) {
    			this.$('#btnAssetRentMngtListExcelDownload').show();
    		}
    		else {
    			this.$('#btnAssetRentMngtListExcelDownload').hide();
    		}
        ...
        switch(buttonId) {
            ...
            case 'addAssetRentRenew':
                ...
                if(rows.length>=1) {
                    ...
                    if( confirm("연장신청을 하시겠습니까?") ) {
                        this.doAction('/asset/rent/gamInsertAssetRentMngtRenew.do', rows[0], function(module, result) {
            ...
            case 'btnSaveItem':
                ...
                if( confirm("저장하시겠습니까?") ) {
                    ...
                    this.doAction('/asset/rent/gamSaveAssetRentMngt.do', inputVO, function(module, result) {
                        if(result.resultCode == 0){
                        	module.loadData();
            ...
            case 'btnRemoveItem':
                ...
                if(rows.length == 0) {
                	...
                	if( confirm("신청삭제를 하시겠습니까?") ) {
                        ...
                        this.doAction('/asset/rent/gamDeleteAssetRentMngt.do', inputVO, function(module, result) {
            ...
            case 'btnSaveComment':
                ...
                this.doAction('/asset/rent/gamUpdateAssetRentMngtComment.do', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                    }
...
//                 this._editRow=this.$('#assetRentDetailList').flexGetData().length;
    ...
    /*                     alert("결재완료 상태가 아닙니다.");
                    ...
                    this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '/asset/rent/popup/showAssetRentMngtPrmisn.do', opts);
            ...
            */
            ...
            case 'btnPrmisnCancel': // 승낙취소
                ...
                if(rows.length>=1) {
                    this.doAction('/asset/rent/checkAssetRentMngtPrmisnCancel.do', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                            if( confirm("승낙취소를 하시겠습니까?") ) {
                                module.doAction('/asset/rent/gamUpdateAssetRentMngtPrmisnCancel.do', rows[0], function(module, result) {
                                    if(result.resultCode=='0') {
                                    	 module.loadData();
                                    }
            ...
            case 'btnNoticeAdit2':
                ...
                if(rows.length>=1) {
                    this.doExecuteDialog('insertLevReqestAdit', '추가 사용료 고지', '/asset/rent/popupLevReqestAdit.do', rows[0]);
                } else {
                    alert("목록에서 선택하십시오.");
                }
...
//                    var userid='admin';
    ...
    */
    		case 'btnUploadFile':
    			this.uploadSingleFile('/asset/rent/uploadRentAttachFile.do', function(module, resp) {
            ...
            case 'btnDownloadFile':
        		...
        		if(selectRow.length > 0) {
        			var row=selectRow[0];
        			this.downloadSingleFile("/asset/rent/downloadRentAttachFile.do", row["physcalFileNm"], row["logicalFileNm"]);
        		}
        		break;
            ...
            case 'btnAssetRentMngtListExcelDownload':	// 엑셀 다운로드
            	this.$('#assetRentMngtList').flexExcelDown('/asset/rent/selectAssetRentMngtListExcel.do');
                break;
    ...
    GamAssetRentMngtModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
    	 switch(newTabId) {
    	    ...
    	    case 'tabs2':
    	    	if(this._cmd!='insert') {
    	    		if(this._param!=null && this._param.action=='AssetInqire') break;
    ...
    GamAssetRentMngtModule.prototype.loadOlnlpList = function(AssetCd) {
        this.doAction('/asset/rent/selectOlnlpInfo.do', AssetCd, function(module, result) {
            if(result.resultCode=='0') {
        	               	...
        	               	<table style="width:100%;">
    	                        <tr>
    	                            <td style="text-align: right">
    	                                ...
    	                                <button id="btnAssetRentMngtListExcelDownload">엑셀</button>
