 ...
 */
function GamCmmnCntrUseExprInqireModule() {}

GamCmmnCntrUseExprInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamCmmnCntrUseExprInqireModule.prototype.loadComplete = function() {

    // 공컨장치장임대 테이블 설정
    this.$("#cmmnCntrUseExprInqireList").flexigrid({
        module: this,
    ...
    // 공컨장치장임대상세 테이블 설정
    this.$("#cmmnCntrUseExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/shed/gamSelectCmmnCntrUseExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#cmmnCntrUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        module.$('#cmd').val('modify');

        module.$('#gamCmmnCntrUseExprInqireForm :input').val('');

        module.makeFormValues('#gamCmmnCntrUseExprInqireForm', row);
        module._editData=module.getFormValues('#gamCmmnCntrUseExprInqireForm', row);
        module._editRow=module.$('#cmmnCntrUseExprInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamCmmnCntrUseExprInqireForm');
        module.$('#cmmnCntrUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#cmmnCntrUseExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds()[0];
    });

    this.$("#cmmnCntrUseExprInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 1});
        module.$('#cmd').val('modify');
        module.$('#gamCmmnCntrUseExprInqireForm :input').val('');
    ...
    this.$("#cmmnCntrUseExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds()[0];
...
GamCmmnCntrUseExprInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamCmmnCntrUseExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'searchBtn':
            ...
            var searchOpt=this.makeFormArgs('#gamCmmnCntrUseExprInqireSearchForm');
            this.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 0});
            this.$('#cmmnCntrUseExprInqireList').flexOptions({params:searchOpt}).flexReload();

            break;

        // 최초신청
        case 'addAssetRentFirst':
            this.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamCmmnCntrUseExprInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#cmmnCntrUseExprInqireDetailList").flexRemove();
            this.$("#cmmnCntrUseExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#cmmnCntrUseExprInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/shed/gamInsertCmmnCntrUseExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCmmnCntrUseExprInqireSearchForm');
                            module.$('#cmmnCntrUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#cmmnCntrUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#cmmnCntrUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamCmmnCntrUseExprInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamCmmnCntrUseExprInqireForm', {_updtId:'I'});
                ...
                this.doAction('<c:url value="/oper/shed/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamCmmnCntrUseExprInqireForm');
                        module.$('#cmmnCntrUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#cmmnCntrUseExprInqireDetailList').flexReload();
                        module.$('#cmmnCntrUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
            ...
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamCmmnCntrUseExprInqireForm');
                    ...
                    this.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamCmmnCntrUseExprInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#cmmnCntrUseExprInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#cmmnCntrUseExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#cmmnCntrUseExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("공컨장치장임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#cmmnCntrUseExprInqireDetailList').flexGetRow(this.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#cmmnCntrUseExprInqireDetailList').flexRemoveRow(this.$('#cmmnCntrUseExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#cmmnCntrUseExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#cmmnCntrUseExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamCmmnCntrUseExprInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamCmmnCntrUseExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCmmnCntrUseExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#cmmnCntrUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamCmmnCntrUseExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#cmmnCntrUseExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#cmmnCntrUseExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
var module_instance = new GamCmmnCntrUseExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamCmmnCntrUseExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="cmmnCntrUseExprInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="cmmnCntrUseExprInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <div class="emdControlPanel"></div>
                    <form id="gamCmmnCntrUseExprInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="cmmnCntrUseExprInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="cmmnCntrUseExprInqireDetailList" style="display:none"></table>
