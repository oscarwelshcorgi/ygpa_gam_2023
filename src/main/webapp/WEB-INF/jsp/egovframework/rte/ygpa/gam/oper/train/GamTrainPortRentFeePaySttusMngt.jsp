 ...
 */
function GamTrainPortRentFeePaySttusMngtModule() {}

GamTrainPortRentFeePaySttusMngtModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamTrainPortRentFeePaySttusMngtModule.prototype.loadComplete = function(params) {

    // 테이블 설정 //
    this.$("#trainPortRentFeePaySttusMngtList").flexigrid({
        module: this,
        url: '/oper/train/gamSelectTrainPortRentFeePaySttusMngtList.do',
        dataType: 'json',
        colModel : [
...
//					{display:'요금종류', name:'chrgeKnd',width:55, sortable:false,align:'center'},
    ...
    */
    ...
    /* 연체내역 목록 */
    this.$("#trainPortRentFeePaySttusArrrgList").flexigrid({
        module: this,
        url: '/oper/train/selectTrainPortRentFeePaySttusMngtDlyList.do',
        dataType: 'json',
        colModel : [
        ...
        preProcess: function(module,data) {
    		if (data.resultCode == "0") {
    			module.makeDivValues('#trainPortRentFeePaySttusMngtListForm',data.resultDlyInfo);	// 리스트 값을 채운다

    			module.makeDivValues('#trainPortRentFeePaySttusMngtSum', data.resultSummary); // 결과값을 채운다.
 ...
 GamTrainPortRentFeePaySttusMngtModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'btnUpdatePayDtls':	// 납부 현황 새로 고침
            this.doAction('/oper/train/updateTrainPortRentFeePaySttusMngtList.do', null, function(module, result) {

                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamTrainPortRentFeePaySttusMngtSearchForm');
        ...
        case 'btnNticArrrgCancelPk':
        	var rows = this.$('#trainPortRentFeePaySttusArrrgList').selectedRows();
			if(rows.length==0) {
        ...
        case 'btnNticIssuePrint':
            ...
            this.printPayNotice('/oper/train/printTrainPortRentFeePayNotice.do', row);
        	break;
        case 'btnNticIssuePrint2':
            ...
            this.printPayNotice('/oper/train/printTrainPortRentFeePayNotice2.do', row);
        	break;
        case 'btnTrainPortRentFeePaySttusMngtListExcelDownload':	// 엑셀 다운로드
    		this.$('#trainPortRentFeePaySttusMngtList').flexExcelDown('/oper/train/selectTrainPortRentFeePaySttusMngtListExcel.do');
    		break;
    }
};
...
//	}
	...
	if(this.resultDetail.arrrgNo!=null) {	// 기 연체 고지된 건이 있는지
	 	this.doAction('/oper/train/updateNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/train/printTrainPortRentFeePayNotice.do', row);
				}
			} else {
				alert(result.resultMsg);
			}
	 	...
	 	this.doAction('/oper/train/insertNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/train/printTrainPortRentFeePayNotice.do', row);
				}
...
GamTrainPortRentFeePaySttusMngtModule.prototype.nticArrrgCancelAll = function() {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/train/cancelNticArrrg.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTrainPortRentFeePaySttusMngtModule.prototype.nticArrrgCancelPk = function(ntic) {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/train/cancelNticArrrgPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTrainPortRentFeePaySttusMngtModule.prototype.nticArrrgPrintCancelPk = function(ntic) {
	var arrrgDetail = [
	               { name: 'prtAtCode', value: ntic.prtAtCode},
	               { name: 'fiscalYr', value: ntic.fiscalYr },
	               { name: 'billNo', value: ntic.billNo },
 	...
 	this.doAction('/oper/train/cancelNticArrrgPrintPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamTrainPortRentFeePaySttusMngtModule.prototype.loadDetailPage = function() {
	...
	var nticDetail = [
	 ...
	 this.doAction('/oper/train/selectTrainPortRentFeePaySttusMngtDetail.do', nticDetail, function(module, result) {
		if (result.resultCode == "0") {
...
GamTrainPortRentFeePaySttusMngtModule.prototype.calcDlyDueDate = function() {
	// 납부기한 계산
...
GamTrainPortRentFeePaySttusMngtModule.prototype.loadArrrgPage = function() {
	var row = this.$('#trainPortRentFeePaySttusMngtList').selectedRows()[0];
	var searchOpt = [];
    ...
    this.$('#trainPortRentFeePaySttusArrrgList').flexOptions({params:searchOpt}).flexReload();
	/*
	var dlyList = [
	               { name: 'prtAtCode', value: row.prtAtCode},
	               { name: 'chrgeKnd', value: row.chrgeKnd },
...
GamTrainPortRentFeePaySttusMngtModule.prototype.getMonthInterval = function (time1,time2,gubun){
	/* 날짜 계산 */
...
GamTrainPortRentFeePaySttusMngtModule.prototype.getDayInterval = function (fdate,tdate){
...
GamTrainPortRentFeePaySttusMngtModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamTrainPortRentFeePaySttusMngtModule();
</script>
<!-- 아래는 고정 -->
...
<div class="window_main">
    ...
    <div class="emdPanel fillHeight">
        <div id="trainPortRentFeePaySttusMngtListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange" data-onchange-before="onTabChangeBefore">
            <ul>
                <li><a href="#tabs1" class="emdTab">철송장임대사용료납부현황 목록</a></li>
                <li><a href="#tabs2" class="emdTab">철송장임대사용료납부현황 상세</a></li>
                <!-- change** -->
                <li><a href="#tabs3" class="emdTab">철송장임대사용료연체현황 목록</a></li>
            </ul>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                ...
                <div id="trainPortRentFeePaySttusMngtListSum" class="emdControlPanel">
					<form id="summaryForm">
			...
			<div id="tabs3" class="emdTabPage" style="overflow: scroll;">
                ...
                <!-- <div class="emdControlPanel"><button id="btnSaveItem">저장</button><button id="btnCancelItem">취소</button><button id="btnRemoveItem">삭제</button></div>  -->
                    <form id="trainPortRentFeePaySttusMngtListForm">
                    	<div>
                    ...
                    <h2>연체 고지 목록</h2>
                    <table id="trainPortRentFeePaySttusArrrgList" style="display:none" class="fillHeight"></table>
                    <table style="width:100%; margin-bottom: 2px" id="trainPortRentFeePaySttusMngtSum" class="summaryPanel">
						<tr>
