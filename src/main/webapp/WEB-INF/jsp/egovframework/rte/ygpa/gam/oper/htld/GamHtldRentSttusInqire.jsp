...
GamHtldRentSttusInqireModule.prototype.loadComplete = function() {
    ...
    this.$("#htldRentSttusInqireList").flexigrid({
        ...
        preProcess: function(module,data) {
            ...
            module.$("#htldRentSttusInqireListTab").tabs("option", {active: 0});    // 탭을 전환 한다.
    ...
    // 배후단지임대상세 테이블 설정
    this.$("#htldRentSttusInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/htld/selectHtldRentSttusInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#htldRentSttusInqireList").on('onItemSelected', function(event, module, row, grid, param) {
    	...
    	module.$('#gamHtldRentSttusInqireForm :input').val('');
    	
    	module.makeFormValues('#gamHtldRentSttusInqireForm', row);
        module._editData=module.getFormValues('#gamHtldRentSttusInqireForm', row);
        module._editRow=module.$('#htldRentSttusInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireForm');
        module.$('#htldRentSttusInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#htldRentSttusInqireFileList').flexOptions({params:searchOpt}).flexReload();
    });
    
    this.$("#htldRentSttusInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        //module.$('#btnApplyGisAssetsCode').prop('disabled', false);
        module.$('#gamHtldRentSttusInqireDetailForm :input').val('');
        
        //module.makeFormValues('#gamHtldRentSttusInqireDetailForm', row);
        module.makeDivValues('#gamHtldRentSttusInqireDetailForm', row);
        module._editData=module.getFormValues('#gamHtldRentSttusInqireDetailForm', row);
        module._editRow=module.$('#htldRentSttusInqireDetailList').selectedRowIds()[0];
    });
    ...
    this.$("#htldRentSttusInqireFileList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module.makeDivValues('#gamHtldRentSttusInqireFileForm', row);
        module._editDataFile=module.getFormValues('#gamHtldRentSttusInqireFileForm', row);
        module._editRowFile=module.$('#htldRentSttusInqireFileList').selectedRowIds()[0];
    ...
    this.$("#htldRentSttusInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#htldRentSttusInqireListTab").tabs("option", {active: 2});
        module.$('#gamHtldRentSttusInqireDetailForm :input').val('');
 ...
 */
 GamHtldRentSttusInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        // 최초신청
        case 'addHtldRentSttusInqireFirst':
            this.$("#htldRentSttusInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            //this.$(":input").val('');
            this.$('#gamHtldRentSttusInqireForm :input').val("");
            this.$("#cmd").val('insert');

            break;

        // 연장신청
        case 'addHtldRentSttusInqireRenew':
            var rows = this.$('#htldRentSttusInqireList').selectedRows();

            if(rows.length>=1) {
                //this.$('#rPrtAtCode').val(row[0]['prtAtCode']);

                this.doAction('<c:url value="/oper/htld/gamInsertHtldRentSttusInqireRenew.do" />', rows[0], function(module, result) {

                    if(result.resultCode=='0') {

                        var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireSearchForm');
                        module.$('#htldRentSttusInqireList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        // 배후단지임대현황 저장
        case 'btnSaveItem':
            var inputVO=this.makeFormArgs('#gamHtldRentSttusInqireForm');
            if(this.$("#cmd").val()=='insert') {

            	this.doAction('<c:url value="/oper/htld/gamInsertHtldRentSttusInqireFirst.do" />', inputVO, function(module, result) {
                ...
                this.doAction('<c:url value="/oper/htld/gamUpdateHtldRentSttusInqire.do" />', inputVO, function(module, result) {
                	if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireForm');
                        module.$('#htldRentSttusInqireList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        case 'btnRemoveItem':
            if( this.$('#cmd').val() == 'modify' ) {
                ...
                this.doAction('<c:url value="/oper/htld/gamDeleteHtldRentSttusInqire.do" />', inputVO, function(module, result) {
                ...
                this.$("#htldRentSttusInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                this.$('#gamHtldRentSttusInqireForm :input').val("");
                this.$("#cmd").val('insert');
        ...
        case 'btnInsertItemDetail':
        	...
        	if( this.$('#prtAtCode').val() == '' ) {
        		alert("배후단지임대현황상세 조회후 등록이 가능합니다.");
        	} else {
        		this.$("#htldRentSttusInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamHtldRentSttusInqireDetailForm :input').val("");
        ...
        case 'btnRemoveItemDetail':
            ...
            var rows = this.$('#htldRentSttusInqireDetailList').selectedRows();

            if(rows.length>=1) {
                this.doAction('<c:url value="/oper/htld/gamDeleteHtldRentSttusInqireDetail.do" />', rows[0], function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireForm');
                        module.$('#htldRentSttusInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        case 'btnSaveItemDetail':
            ...
            var inputVO=this.makeFormArgs('#gamHtldRentSttusInqireDetailForm');
            if(this.$("#detailCmd").val()=='insert') {

                this.doAction('<c:url value="/oper/htld/gamInsertHtldRentSttusInqireDetail.do" />', inputVO, function(module, result) {
            	...
            	this.doAction('<c:url value="/oper/htld/gamUpdateHtldRentSttusInqireDetail.do" />', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                    	var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireForm');
                        module.$('#htldRentSttusInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        case 'popupEntrpsInfoInput': // 팝업을 호출한다.(배후단지임대현황입력)
            var opts;
        ...
        case 'btnPrmisn': // 승낙 (허가)
            ...
            if(rows.length>=1) {
                ...
                this.doExecuteDialog('insertHtldRentSttusInqirePrmisnPopup', '승낙', '<c:url value="/oper/htld/popup/showHtldRentSttusInqirePrmisn.do"/>', opts);
        ...
        case 'btnPrmisnCancel': // 승낙취소 (허가취소)
            ...
            if(rows.length>=1) {
                this.doAction('<c:url value="/oper/htld/gamUpdateHtldRentSttusInqirePrmisnCancel.do" />', rows[0], function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamHtldRentSttusInqireForm');
                        module.$('#htldRentSttusInqireList').flexOptions({params:searchOpt}).flexReload();
                    }
...
GamHtldRentSttusInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    ...
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#htldRentSttusInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
    	var row = this.$('#htldRentSttusInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamHtldRentSttusInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
	switch (popupId) {
     ...
     case 'insertEntrpsInfoPopup':
         ...
         break;
     case 'insertHtldRentSttusInqirePrmisnPopup':
    	 if (msg != 'cancel') {
    		 if( value == "0" ) {
    			 var searchOpt=this.makeFormArgs('#gamHtldRentSttusInqireSearchForm');
                 this.$('#htldRentSttusInqireList').flexOptions({params:searchOpt}).flexReload();
...
<div class="window_main">
    ...
    <div class="emdPanel fillHeight">
        <div id="htldRentSttusInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">배후단지임대현황 목록</a></li>
                <li><a href="#tabs2" class="emdTab">배후단지임대현황 내역</a></li>
                <li><a href="#tabs3" class="emdTab">배후단지임대현황 상세내역</a></li>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <!-- <div class="emdControlPanel"></div>-->
                    <form id="gamHtldRentSttusInqireForm">
                 ...
                 <table id="htldRentSttusInqireDetailList" style="display:none"></table>

                 <table style="width:100%">
                 	<!-- 
                    <tr>
                        <td style="text-align:right" colspan="3"><button id="btnInsertItemDetail">배후단지임대상세추가</button><button id="btnRemoveItemDetail">배후단지임대상세삭제</button></td>
                    </tr>
                     -->
                    <!--
                    <tr>
            ...
            <div id="tabs3" class="emdTabPage" style="overflow: scroll;">
                ...
                <!-- <div class="emdControlPanel"><button id="btnSaveItemDetail">저장</button></div> -->
                    <form id="gamHtldRentSttusInqireDetailForm">
