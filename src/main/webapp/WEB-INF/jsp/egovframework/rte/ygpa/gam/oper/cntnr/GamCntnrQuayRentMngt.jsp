 ...
 * @Description : 컨테이너부두임대사용목록관리 (컨테이너부두임대/일반부두/컨테이너부두임대사용목록관리)
 ...
 * since 2014.01.14
 *
 * Copyright (C) 2013 by LFIT  All right treserved.
 */
%>
...
GamAssetRentMngtModule.prototype.loadComplete = function() {
    ...
    this.$("#assetRentMngtList").flexigrid({
        module: this,
        url: '/oper/cntnr/gamSelectCntnrQuayRentMngtList.do',
        dataType: 'json',
        colModel : [
...
//                    {display:'신청업체', name:'entrpscd',width:60, sortable:false,align:'center'},
    ...
    this.$("#assetRentDetailList").flexigrid({
        module: this,
        url: '/oper/cntnr/gamSelectCntnrQuayRentMngtDetailList.do',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '/oper/cntnr/gamSelectCntnrQuayRentMngtFileList.do',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        if(row.filenmPhysicl!=null || row.filenmPhysicl!='') {
			...
			if(ext=='jpg' || ext=='jpeg' || ext=='bmp' || ext=='png' || ext=='gif') {
			    $imgURL = module.getUrl("/oper/cntnr/getRentAttachFile.do?physicalFileNm=")+filenm;
...
//            this.$("#assetRentFileList").flexAddData({resultList:[]}); //그리드 초기화
        ...
        case 'addAssetRentRenew':
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('/oper/cntnr/gamInsertCntnrQuayRentMngtRenew.do', rows[0], function(module, result) {
        ...
        case 'btnSaveItem':
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                this.doAction('/oper/cntnr/gamSaveCntnrQuayRentMngt.do', inputVO, function(module, result) {
        ...
        case 'btnRemoveItem':
            ...
            if(rows.length == 0) {
            	...
            	if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    this.doAction('/oper/cntnr/gamDeleteCntnrQuayRentMngt.do', inputVO, function(module, result) {
        ...
        case 'btnSaveComment':
            ...
            this.doAction('/oper/cntnr/gamUpdateCntnrQuayRentMngtComment.do', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                }
...
/*                     alert("결재완료 상태가 아닙니다.");
				...
				console.log(opts);
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '/oper/cntnr/popup/showCntnrQuayRentMngtPrmisn.do', opts);
        ...
        */
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                if( confirm("승낙취소를 하시겠습니까?") ) {
                    this.doAction('/oper/cntnr/gamUpdateCntnrQuayRentMngtPrmisnCancel.do', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                        	 module.loadData();
                        }
        ...
        case 'btnNoticeAdit2':
            ...
            if(rows.length>=1) {
                this.doExecuteDialog('insertLevReqestAdit', '추가 사용료 고지', '/oper/cntnr/popupLevReqestAdit.do', rows[0]);
            } else {
                alert("목록에서 선택하십시오.");
            }
...
//              var userid=EMD.util.getLoginUserVO().userNm; 임시
        ...
        case 'btnDownloadFile':
    		...
    		if(selectRow.length > 0) {
    			var row=selectRow[0];
    			this.downloadSingleFile("/oper/cntnr/downloadRentAttachFile.do", row["filenmPhysicl"]);
    		}
    		break;
        ...
        case 'btnCntnrQuayRentMngtListExcelDownload':	// 엑셀 다운로드
        	this.$('#assetRentMngtList').flexExcelDown('/oper/cntnr/selectCntnrQuayRentMngtListExcel.do');
            break;
        ...
        case 'btnRentFeeMngt':
            ...
            if(rows.length>0) {
       	 	...
       	 	EMD.util.create_window('컨테이너부두임대사용료관리', '/oper/cntnr/gamCntnrQuayRentFeeMngt.do', null, opts);
        	break;
    }
};
    	               	...
    	               	<table style="width:100%;">
	                        <tr>
	                            <td style="text-align: right">
	                                ...
	                                <button id="btnCntnrQuayRentMngtListExcelDownload">엑셀</button>
