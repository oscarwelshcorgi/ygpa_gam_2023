...
<%
  /**
  * @Class Name : GamMarineCenterUseExprInqire.jsp
 ...
 */
function GamMarineCenterUseExprInqireModule() {}

GamMarineCenterUseExprInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamMarineCenterUseExprInqireModule.prototype.loadComplete = function() {

    // 마린센터임대 테이블 설정
    this.$("#marineCenterUseExprInqireList").flexigrid({
        module: this,
        url: '<c:url value="/oper/center/gamSelectMarineCenterUseExprInqireList.do" />',
        dataType: 'json',
        colModel : [
    ...
    // 마린센터임대상세 테이블 설정
    this.$("#marineCenterUseExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/center/gamSelectMarineCenterUseExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '<c:url value="/oper/center/gamSelectAssetRentFileList.do" />',
        dataType: 'json',
        colModel : [
					/*
    ...
    this.$("#marineCenterUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#marineCenterUseExprInqireList').selectedRowIds()[0];

        //해당하는 마린센터임대상세 목록과 파일목록를 불러온다.
        ...
        module.$('#marineCenterUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#marineCenterUseExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#marineCenterUseExprInqireDetailList').selectedRowIds()[0];
    });

    this.$("#marineCenterUseExprInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
    ...
    this.$("#marineCenterUseExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
...
GamMarineCenterUseExprInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamMarineCenterUseExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            ...
            //this.$("#marineCenterUseExprInqireDetailList").flexRemove();
            this.$("#marineCenterUseExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#marineCenterUseExprInqireList').selectedRows();

            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 마린센터임대관리 메뉴에서 연장신청이 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/center/gamInsertMarineCenterUseExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamMarineCenterUseExprInqireSearchForm');
                            module.$('#marineCenterUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            if( this.$('#quayGroupCd').val() != 'P' ) {
                alert("해당 건은 마린센터임대관리 메뉴에서 저장이 불가능합니다.");
                return;
            }
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#marineCenterUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#marineCenterUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                this.doAction('<c:url value="/oper/center/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamAssetRentForm');
                        module.$('#marineCenterUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#marineCenterUseExprInqireDetailList').flexReload();
                        module.$('#marineCenterUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
        ...
        case 'btnRemoveItem':
            ...
            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 마린센터임대관리 메뉴에서 신청삭제가 불가능합니다.");
                return;
            }
            */

            if(rows.length == 0) {
                alert("마린센터임대목록에서 신청삭제할 행을 선택하십시오.");
            } else {
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    this.doAction('<c:url value="/oper/center/gamDeleteAssetRent.do" />', inputVO, function(module, result) {
        ...
        case 'btnSaveComment':
            ...
            this.doAction('<c:url value="/oper/center/gamUpdateAssetRentComment.do" />', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamMarineCenterUseExprInqireSearchForm');
                    module.$('#marineCenterUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this._editRow=this.$('#marineCenterUseExprInqireDetailList').flexGetData().length;
            }

            break;

        // 마린센터임대상세 삭제 (Grid상에서만 삭제됨)
        case 'btnRemoveItemDetail':
            var rows = this.$('#marineCenterUseExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("마린센터임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#marineCenterUseExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#marineCenterUseExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#marineCenterUseExprInqireDetailList').flexGetRow(this.$('#marineCenterUseExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#marineCenterUseExprInqireDetailList').flexRemoveRow(this.$('#marineCenterUseExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        case 'btnSaveItemDetail':
            ...
            if(this.$("#detailCmd").val()=='insert') {
                ...
                this.doAction('<c:url value="/oper/center/gamInsertAssetRentDetail.do" />', {aaa : "rrrrrrrrrrrrrr"}, function(module, result) {
                ...
                this.doAction('<c:url value="/oper/center/gamUpdateAssetRentDetail.do" />', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamAssetRentForm');
                        module.$('#marineCenterUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'popupEntrpsInfoInput': // 팝업을 호출한다.(마린센터임대입력)
            var opts;
        ...
        case 'btnPrmisn': // 사용승낙
            ...
            if(rows.length>=1) {
                ...
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '<c:url value="/oper/center/popup/showAssetRentPrmisn.do"/>', opts);
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                if( confirm("승낙취소를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/center/gamUpdateAssetRentPrmisnCancel.do" />', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamAssetRentForm');
                            module.$('#marineCenterUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#marineCenterUseExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#marineCenterUseExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamMarineCenterUseExprInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamMarineCenterUseExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamMarineCenterUseExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#marineCenterUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamMarineCenterUseExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#marineCenterUseExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#marineCenterUseExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamMarineCenterUseExprInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
 // 자산코드 조회
...
var module_instance = new GamMarineCenterUseExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamMarineCenterUseExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="assetRentListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">마린센터임대(만기도래) 목록</a></li>
                <li><a href="#tabs2" class="emdTab">마린센터임대내역</a></li>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="marineCenterUseExprInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="marineCenterUseExprInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="marineCenterUseExprInqireDetailList" style="display:none"></table>
