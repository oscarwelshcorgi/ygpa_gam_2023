...
GamAssetRentMngtModule.prototype.loadComplete = function(param) {
    ...
    this.$("#assetRentMngtList").flexigrid({
        module: this,
        url: '/oper/gnrltest/gamSelectPrtFcltyRentMngtList.do',
        dataType: 'json',
        colModel : [
...
//                    {display:'결재일시', name:'sanctnDt',width:110, sortable:false,align:'center'},
    ...
    this.$("#assetRentDetailList").flexigrid({
        module: this,
        url: '/oper/gnrltest/gamSelectPrtFcltyRentMngtDetailList.do',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '/oper/gnrltest/gamSelectPrtFcltyRentMngtFileList.do',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        if(row.filenmPhysicl!=null || row.filenmPhysicl!='') {
			...
			if(ext=='jpg' || ext=='jpeg' || ext=='bmp' || ext=='png' || ext=='gif') {
			    $imgURL = module.getUrl("/oper/gnrltest/getRentAttachFile.do?physicalFileNm=")+filenm;
...
GamAssetRentMngtModule.prototype.loadRentMaster = function(loadOpt) {
    ...
    this.doAction('/oper/gnrltest/selectRentMasterInfo.do', EMD.util.objectToArray(loadOpt), function(module, result) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentRenew':
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('/oper/gnrltest/gamInsertPrtFcltyRentMngtRenew.do', rows[0], function(module, result) {
        ...
        case 'btnSaveItem':
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                this.doAction('/oper/gnrltest/gamSavePrtFcltyRentMngt.do', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                    	module.loadData();
        ...
        case 'btnRemoveItem':
            ...
            if(rows.length == 0) {
            	...
            	if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    this.doAction('/oper/gnrltest/gamDeletePrtFcltyRentMngt.do', inputVO, function(module, result) {
        ...
        case 'btnSaveComment':
            ...
            this.doAction('/oper/gnrltest/gamUpdatePrtFcltyRentMngtComment.do', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                }
...
/*                     alert("결재완료 상태가 아닙니다.");
                ...
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '/oper/gnrltest/popup/showPrtFcltyRentMngtPrmisn.do', opts);
        ...
        */
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                this.doAction('/oper/gnrltest/checkPrtFcltyRentMngtPrmisnCancel.do', rows[0], function(module, result) {
                    if(result.resultCode=='0') {
                        if( confirm("승낙취소를 하시겠습니까?") ) {
                            module.doAction('/oper/gnrltest/gamUpdatePrtFcltyRentMngtPrmisnCancel.do', rows[0], function(module, result) {
                                if(result.resultCode=='0') {
                                	 module.loadData();
                                }
        ...
        case 'btnNoticeAdit2':
            ...
            if(rows.length>=1) {
                this.doExecuteDialog('insertLevReqestAdit', '추가 사용료 고지', '/oper/gnrltest/popupLevReqestAdit.do', rows[0]);
            } else {
                alert("목록에서 선택하십시오.");
            }
...
*/
		case 'btnUploadFile':
			this.uploadSingleFile('/oper/gnrltest/uploadRentAttachFile.do', function(module, resp) {
        ...
        case 'btnDownloadFile':
    		...
    		if(selectRow.length > 0) {
    			var row=selectRow[0];
    			this.downloadSingleFile("/oper/gnrltest/downloadRentAttachFile.do", row["physcalFileNm"], row["logicalFileNm"]);
    		}
    		break;
        ...
        case 'btnPrtFcltyRentMngtListExcelDownload':	// 엑셀 다운로드
        	this.$('#assetRentMngtList').flexExcelDown('/oper/gnrltest/selectPrtFcltyRentMngtListExcel.do');
            break;
