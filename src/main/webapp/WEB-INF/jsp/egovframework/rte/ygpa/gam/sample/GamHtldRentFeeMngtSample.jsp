...
<%
  /**
  * @Class Name : GamHtldRentFeeMngtSample.jsp
  * @Description : 배후단지임대료관리(샘플)
  * @Modification Information
  *
  *   수정일          수정자                   수정내용
  *  -------    --------    ---------------------------
  *  2014.01.14  domh          최초 생성
  *  2016.01.04 김종민   		전면 수정
  *
  * author domh
  * since 2014.01.14
  *
...
--%>
function GamHtldRentFeeMngtModuleSample() {}

<%--
	EmdModule을 상속하여 모듈 클래스를 정의한다.
--%>
GamHtldRentFeeMngtModuleSample.prototype = new EmdModule(1070, 600);
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.loadComplete = function(params) {
    // 사용료고지목록 그리드 설정
    this.$("#assetRentFeeList").flexigrid({
        module: this,
        url: '/sample/gamSelectHtldRentFeeMngtList.do',
        dataType: 'json',
        colModel : [
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onSubmit = function() {
    this.loadData();
};
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onButtonClick = function(buttonId) {
	switch(buttonId) {
	case 'searchBtn': //조회
		this.loadData();
		break;
	...
	case 'btnCancelNticIssue':	// 고지 취소
   	...
   	case 'btnHwpDownload':	// HWP
   		this.printPage('/sample/gamHtldRentHwpPreview.do', {});
   		break;
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onClosePopup = function(popupId, msg, value) {
	switch (popupId) {
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	return true;
};
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onTabChange = function(newTabId, oldTabId) {
};

<%--
	각 요소에 대한 이벤트 설정
--%>
GamHtldRentFeeMngtModuleSample.prototype.setEvents = function() {
    //조회조건의 업체코드가 바뀔 때
...
<%--
	버튼 상태 설정
--%>
GamHtldRentFeeMngtModuleSample.prototype.setButtonStatus = function() {
...
<%--
	사용료고지목록 조회
--%>
GamHtldRentFeeMngtModuleSample.prototype.loadData = function() {
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.makeRowData = function(item) {
...
<%--
	사용료고지목록 합계 표시
--%>
GamHtldRentFeeMngtModuleSample.prototype.onCalcSummary = function() {
	var sum = {
		sumCnt:0,
		sumFee:0,
		sumVat:0,
...
--%>
GamHtldRentFeeMngtModuleSample.prototype.onCalcFeeListCellEdited = function(row, rid, cid) {
	if(row.nhtIsueYn=='Y') {
...
<%--
	고지팝업 오픈
--%>
GamHtldRentFeeMngtModuleSample.prototype.openNticIssuePopup = function() {
    ...
    this.doExecuteDialog('nticIssuePopup', '사용료 고지', '/sample/showNticIssuePopup.do', row);
};

<%--
	고지처리
--%>
GamHtldRentFeeMngtModuleSample.prototype.processNticIssue = function(value) {
	var arg = EMD.util.objectToArray(value);
	this.doAction('/sample/insertHtldFeeNticSingle.do', arg, function(module, result) {
...
<%--
	고지취소
--%>
GamHtldRentFeeMngtModuleSample.prototype.cancelNticIssue = function() {
	if(this.$('#assetRentFeeList').selectedRowCount()<1) {
    ...
    if( confirm("선택한 건의 고지를 취소 하시겠습니까?") ) {
    	if(rows['nhtPrintYn']!='Y' || confirm("해당 건의 징수의뢰 자료가 삭제됩니다. 계속 하시겠습니까?")) {
        	this.doAction('/sample/cancelHtldRentFeeNticSingle.do', row, function(module, result) {
            	if(result.resultCode=='0') {
...
<%--
	납부관리현황 호출
--%>
GamHtldRentFeeMngtModuleSample.prototype.loadRentFeePayMngt = function() {
    var rows = this.$('#assetRentFeeList').selectedRows();
    var opts = {};
    if(rows.length>0) {
    	opts = {
	...
	EMD.util.create_window('gamHtldRentFeePaySttusMngt', '배후단지납부현황관리', '/sample/gamHtldRentFeePaySttusMngt.do', null, opts);
};

<%--
	기간에 따른 이자 계산
--%>
GamHtldRentFeeMngtModuleSample.prototype.getIntrAmount = function(fee, intrRate, nticMth, nticPdFrom, nticPdTo, grUsagePdFrom, grUsagePdTo) {
	var result = 0;
...
<%--
	Date객체 변형 객체 정의.
--%>
GamHtldRentFeeMngtModuleSample.prototype.createExtendedDate = function (argDate) {
	var dateObj = argDate;
...
--%>
var module_instance = new GamHtldRentFeeMngtModuleSample();
</script>
<%--
