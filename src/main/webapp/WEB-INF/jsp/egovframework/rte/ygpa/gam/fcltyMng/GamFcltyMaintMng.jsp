...
<%
  ...
  */
%>
<%-- <validator:javascript formName="gamFcltyMaintCode" staticJavascript="false" xhtml="true" cdata="false" />
<validator:javascript formName="gamFcltyMaintPhoto" staticJavascript="false" xhtml="true" cdata="false" /> --%>
<script>
/*
 * 아래 모듈은 고유 함수명으로 동작 함. 동일한 이름을 사용 하여도 관계 없음.
 */
function GamFcltyMaintMngtModule() {
}

GamFcltyMaintMngtModule.prototype = new EmdModule(1000,600);	// 초기 시작 창크기 지정

// 페이지가 호출 되었을때 호출 되는 함수
GamFcltyMaintMngtModule.prototype.loadComplete = function(params) {
	...
	this.$("#fcltyMngMngtList").flexigrid({
		...
		colModel : [
					...
					{display:"유지보수예산", 		name:"mntnRprBdgt",				width:150, 		sortable:false,		align:"center"},
	...
	this.$("#mntnRprObjFcltsF").flexigrid({
		...
		colModel : [
					...
					{display:"유지보수공법",		name:"strySe",			width:80,		sortable:false,		align:"center"},
...
GamFcltyMaintMngtModule.prototype.removeGisAssetPhotoItem = function() {
...
GamFcltyMaintMngtModule.prototype.clearCodePage = function() {
	this.$('#fcltyMngManageVO :input').val('');
};

GamFcltyMaintMngtModule.prototype.clearPhotoPage = function() {
	this.$('#fcltyMngPhotoList').flexEmptyData();
	this.$('#fcltyMngGisPhotoForm :input').val('');
	this.$('#previewImage').attr('src', '');
};

GamFcltyMaintMngtModule.prototype.loadPhotoList = function() {
	...
	var searchOpt = [
	                ...
	                { name: 'gisPrtFcltyMaintCd', value: row['gisPrtFcltyMaintCd'] },
	                { name: 'gisPrtFcltyMaintSeq', value: row['gisPrtFcltyMaintSeq'] },
	                { name: 'prtFcltyMaintSe', value: row['prtFcltyMaintSe'] }
	              ];
	this.clearPhotoPage();
 ...
 * 탭 변경시 실행 이벤트
 */
 GamFcltyMaintMngtModule.prototype.onTabChange = function(newTabId, oldTabId) {
	switch(newTabId) {
	case "tabs1":
		break;
	case "tabs2":
		...
		if(this._cmd!="insert") {
			...
			if(row.length <= 0) {
		 		this.clearCodePage();
				this.$("#prtFcltyMaintSeNm").hide();
		 		if(this._params.action!=null || this._params.action=='prtFcltyMaintMngt') {
		 			var prtFcltyMaint = [
		 			                ...
		 			                { name: 'gisPrtFcltyMaintCd', value: this._params.gisPrtFcltyMaintCd },
		 			                { name: 'gisPrtFcltyMaintSeq', value: this._params.gisPrtFcltyMaintSeq },
		 			                { name: 'prtFcltyMaintSe', value: this._params.prtFcltyMaintSe }
		 			              ];
		 	     	 	this.doAction('<c:url value="/fcltyMng/gamElctyFcltyMaintDetail.do" />', prtFcltyMaint, function(module, result) {
		 	     	 		if(result.resultCode == "0"){
		 	     	 			module.clearCodePage();
		 	     	 			module._fcltyMngItem=result.result;
		 	     	 			module.makeFormValues('#fcltyMngManageVO', result.result);	// 결과값을 채운다.
		 	     	 			module.$("#beforeGisPrtFcltyMaintCd").val(module.$("#gisPrtFcltyMaintCd").val());
			                    module.$("#beforeGisPrtFcltyMaintSeq").val(module.$("#gisPrtFcltyMaintSeq").val());

			                    if(result.result.gisPrtFcltyMaintCd==10){
			                    	module.$("#gisPrtFcltyMaintCdSub").empty();
			                    	module.$("#gisPrtFcltyMaintCdSub").append('<option value="가로등" id="가로등">가로등</option><option value="조명탑" id="조명탑">조명탑</option><option value="신호등" id="신호등">신호등</option><option value="보안등" id="보안등">보안등</option><option value="기타" id="기타">기타</option>');

			                     }else if(result.result.gisPrtFcltyMaintCd==11){
			                     	module.$("#gisPrtFcltyMaintCdSub").empty();
			                     	module.$("#gisPrtFcltyMaintCdSub").append('<option value="이동식" id="이동식">이동식</option><option value="고정식" id="고정식">고정식</option><option value="기타" id="기타">기타</option>');

			                     }else if(result.result.gisPrtFcltyMaintCd==12){
			                     	module.$("#gisPrtFcltyMaintCdSub").empty();
			                     	module.$("#gisPrtFcltyMaintCdSub").append('<option value="태양광" id="태양광">태양광</option><option value="풍력" id="풍력">풍력</option><option value="지력" id="지력">지력</option><option value="수력" id="수력">수력</option><option value="기타" id="기타">기타</option>');

			                     }else{
			                     	module.$("#gisPrtFcltyMaintCdSub").empty();
			                     	module.$("#gisPrtFcltyMaintCdSub").append('<option value="기타" id="기타">기타</option>');
			                     }
			                    module.$("#gisPrtFcltyMaintCdSub").find("#"+result.result.gisPrtFcltyMaintCdSub).attr("selected", "selected");
		 	     	 		}
		 	     	 		else {
			...
			if(row.length <= 0) {
		 		...
		 		if(this._params.action!=null || this._params.action=='prtFcltyMaintMngt') {
		 			var prtFcltyMaint = [
		 		 	...
		 		 	this.$('#fcltyMngPhotoList').flexOptions({params:prtFcltyMaint}).flexReload();
	 	     	 	}
				return;
			}
			row=row[0];
 ...
 * 팝업 close 이벤트
 */
GamFcltyMaintMngtModule.prototype.onClosePopup = function(popupId, msg, value){
	...
	switch(popupId){
		...
		case "searchEntrpsCdPopup":
			this.$("#prtFcltyMaintMngEntrpsCd").val(value["entrpscd"]);
			this.$("#prtFcltyMaintMngEntrpsNm").val(value["entrpsNm"]);
		break;
...
var module_instance = new GamFcltyMaintMngtModule();
</script>
<!-- 아래는 고정 -->
				...
				<table class="searchPanel">
					<tbody>
						<tr>
							...
							<th>유지보수공사명</th>
							<td><input type="text" id="searchFcltyMaintCd" size="50" /></td>
