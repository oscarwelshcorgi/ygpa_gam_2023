...
GamHtldRentFeeMngtModule.prototype = new EmdModule(800, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamHtldRentFeeMngtModule.prototype.loadComplete = function() {

    // 테이블 설정 //       
    this.$("#htldRentFeeMngtList").flexigrid({
        module: this,
    ...
    this.$("#htldRentFeeMngtList").on('onItemSelected', function(event, module, row, grid, param) {
        module.$('#cmd').val('modify');

        module.$('#gamHtldRentFeeMngtForm :input').val('');

        module.makeFormValues('#gamHtldRentFeeMngtForm', row);
        module._editData=module.getFormValues('#gamHtldRentFeeMngtForm', row);
        module._editRow=module.$('#htldRentFeeMngtList').selectedRowIds()[0];
    });
    
    this.$("#htldRentFeeMngtList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        // 이벤트내에선 모듈에 대해 선택한다.
        module.$("#htldRentFeeMngtListTab").tabs("option", {active: 1});    // 탭을 전환 한다.
        this.$("#cmd").val('modify');
 ...
 GamHtldRentFeeMngtModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'searchBtn':
        	...
        	this.$("#htldRentFeeMngtListTab").tabs("option", {active: 0}); //2014-4-22 추가
        ...
        case 'btnInsertItem':
        	module.$("#htldRentFeeMngtListTab").tabs("option", {active: 1});    // 탭을 전환 한다.
            this.$('#gamHtldRentFeeMngtForm').find(':input').val('');
            this.$("#cmd").val('insert');
        ...
        case 'btnSaveItem':
        	...
        	if( this.$('#prtAtCode').val() == '' ) {
	        	...
	        	var inputVO=this.makeFormArgs('#gamHtldRentFeeMngtForm');
	        	
	        	this.doAction('<c:url value="/oper/htld/gamUpdateHtldRentFeeMngt.do" />', inputVO, function(module, result) {
	
	                if(result.resultCode=='0') {
	                    var searchOpt=module.makeFormArgs('#gamHtldRentFeeMngtSearchForm');
	                    module.$('#htldRentFeeMngtList').flexOptions({params:searchOpt}).flexReload();
	                }
        	...
        	*/
        	...
        	if( this.$("#cmd").val() == 'insert' ) {
                ...
                this.doAction('<c:url value="/oper/htld/gamInsertHtldRentFeeMngt.do" />', inputVO, function(module, result) {
         ...
         // 취소
        case 'btnCancelItem':
            this.$('#gamHtldRentFeeMngtForm :input').val("");
            break;
        ...
        case 'saveNticListBtn':
            ...
            var reglist = this.$("#htldRentFeeMngtList").selectFilterData(filter);
            ...
            if(reglist.length > 0){
                ...
                this.doAction('<c:url value="/oper/htld/gamInsertHtldRentFeeMngtNtic.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt = module.makeFormArgs("#gamHtldRentFeeMngtSearchForm");
                        module.$("#htldRentFeeMngtList").flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'cancelNticListBtn': 
            ...
            if(reglist.length > 0){
                ...
                var inputVO = {nticCnts: nticCnts, prtAtCodes: prtAtCodes, mngYears: mngYears, mngNos: mngNos, mngCnts: mngCnts, nticnos: nticnos, accnutYears: accnutYears };
                this.doAction('<c:url value="/oper/htld/gamDeleteHtldRentFeeMngtNtic.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt = module.makeFormArgs("#gamHtldRentFeeMngtSearchForm");
                        module.$("#htldRentFeeMngtList").flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'saveNticDetailBtn':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.doAction('<c:url value="/oper/htld/gamInsertHtldRentFeeMngtNticSingle.do" />', inputVO, function(module, result) {
        ...
        case 'cancelNticDetailBtn':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.doAction('<c:url value="/oper/htld/gamDeleteHtldRentFeeMngtNticSingle.do" />', inputVO, function(module, result) {
        ...
        */
        ...
        case 'saveNticDetailBtn':
        	var rows = this.$('#htldRentFeeMngtList').selectedRows();
        	
        	if(rows.length>=1) {
        		if( confirm("고지의뢰를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/htld/gamInsertHtldRentFeeMngtNticSingle.do" />', rows[0], function(module, result) {
        ...
        case 'cancelNticDetailBtn':
            ...
            if(rows.length>=1) {
                if( confirm("고지취소를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/htld/gamDeleteHtldRentFeeMngtNticSingle.do" />', rows[0], function(module, result) {
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamHtldRentFeeMngtSearchForm">
        ...
        <div id="htldRentFeeMngtListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <!-- <div style="width: 100%; height: 100%; overflow:auto">  -->
                        <table id="htldRentFeeMngtList" style="display:none" class="fillHeight"></table>
                <!-- </div>  -->
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                ...
                <div class="emdControlPanel">
                    ...
                    <form id="gamHtldRentFeeMngtForm">
