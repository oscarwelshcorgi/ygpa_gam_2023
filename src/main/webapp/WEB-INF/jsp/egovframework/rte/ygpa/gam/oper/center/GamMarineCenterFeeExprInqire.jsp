...
<%
  /**
  * @Class Name : GamMarineCenterFeeExprInqire.jsp
 ...
 */
function GamMarineCenterFeeExprInqireModule() {}

GamMarineCenterFeeExprInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamMarineCenterFeeExprInqireModule.prototype.loadComplete = function() {

    // 마린센터임대 테이블 설정
    this.$("#marineCenterFeeExprInqireList").flexigrid({
        module: this,
        url: '<c:url value="/oper/center/gamSelectMarineCenterFeeExprInqireList.do" />',
        dataType: 'json',
        colModel : [
    ...
    // 마린센터임대상세 테이블 설정
    this.$("#marineCenterFeeExprInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/center/gamSelectMarineCenterFeeExprInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#marineCenterFeeExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#marineCenterFeeExprInqireList').selectedRowIds()[0];
        ...
        module.$('#marineCenterFeeExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#marineCenterFeeExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#marineCenterFeeExprInqireDetailList').selectedRowIds()[0];
    });

    this.$("#marineCenterFeeExprInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
    ...
    this.$("#marineCenterFeeExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
...
GamMarineCenterFeeExprInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamMarineCenterFeeExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            ...
            //this.$("#marineCenterFeeExprInqireDetailList").flexRemove();
            this.$("#marineCenterFeeExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#marineCenterFeeExprInqireList').selectedRows();
            ...
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/center/gamInsertMarineCenterFeeExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamMarineCenterFeeExprInqireSearchForm');
                            module.$('#marineCenterFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#marineCenterFeeExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#marineCenterFeeExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                this.doAction('<c:url value="/oper/center/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamAssetRentForm');
                        module.$('#marineCenterFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#marineCenterFeeExprInqireDetailList').flexReload();
                        module.$('#marineCenterFeeExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this._editRow=this.$('#marineCenterFeeExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#marineCenterFeeExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("마린센터임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#marineCenterFeeExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#marineCenterFeeExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#marineCenterFeeExprInqireDetailList').flexGetRow(this.$('#marineCenterFeeExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#marineCenterFeeExprInqireDetailList').flexRemoveRow(this.$('#marineCenterFeeExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        */
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#marineCenterFeeExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#marineCenterFeeExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamMarineCenterFeeExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamMarineCenterFeeExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#marineCenterFeeExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamMarineCenterFeeExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#marineCenterFeeExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#marineCenterFeeExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamMarineCenterFeeExprInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamMarineCenterFeeExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamMarineCenterFeeExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="assetRentListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="marineCenterFeeExprInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="marineCenterFeeExprInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="marineCenterFeeExprInqireDetailList" style="display:none"></table>
