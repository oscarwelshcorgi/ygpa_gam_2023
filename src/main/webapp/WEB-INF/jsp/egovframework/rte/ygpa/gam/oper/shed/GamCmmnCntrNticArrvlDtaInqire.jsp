 ...
 */
function GamCmmnCntrNticArrvlDtaInqireModule() {}

GamCmmnCntrNticArrvlDtaInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamCmmnCntrNticArrvlDtaInqireModule.prototype.loadComplete = function() {

    // 공컨장치장임대 테이블 설정
    this.$("#cmmnCntrNticArrvlDtaInqireList").flexigrid({
        module: this,
        url: '<c:url value="/oper/shed/gamSelectCmmnCntrNticArrvlDtaInqireList.do" />',
        dataType: 'json',
        colModel : [
					...
					{display:'사용료', name:'fee',width:100, sortable:false,align:'right'},
					...
					{display:'총사용료', name:'grFee',width:100, sortable:false,align:'right'},
    ...
    // 공컨장치장임대상세 테이블 설정
    this.$("#cmmnCntrNticArrvlDtaInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/shed/gamSelectCmmnCntrNticArrvlDtaInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '<c:url value="/oper/shed/gamSelectAssetRentFileList.do" />',
        dataType: 'json',
        colModel : [
					/*
    ...
    this.$("#cmmnCntrNticArrvlDtaInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module.$('#gamCmmnCntrNticArrvlDtaInqireForm :input').val('');

        module.makeFormValues('#gamCmmnCntrNticArrvlDtaInqireForm', row);
        module._editData=module.getFormValues('#gamCmmnCntrNticArrvlDtaInqireForm', row);
        module._editRow=module.$('#cmmnCntrNticArrvlDtaInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireForm');
        module.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#cmmnCntrNticArrvlDtaInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds()[0];
    });
    ...
    this.$("#cmmnCntrNticArrvlDtaInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds()[0];
...
GamCmmnCntrNticArrvlDtaInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamCmmnCntrNticArrvlDtaInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'searchBtn':            
            ...
            var searchOpt=this.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireSearchForm');
            this.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 0});
            this.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();

            break;

        // 최초신청
        case 'addAssetRentFirst':
            this.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamCmmnCntrNticArrvlDtaInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#cmmnCntrNticArrvlDtaInqireDetailList").flexRemove();
            this.$("#cmmnCntrNticArrvlDtaInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#cmmnCntrNticArrvlDtaInqireList').selectedRows();

            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 공컨장치장임대관리 메뉴에서 연장신청이 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/shed/gamInsertCmmnCntrNticArrvlDtaInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireSearchForm');
                            module.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            if( this.$('#quayGroupCd').val() != 'P' ) {
                alert("해당 건은 공컨장치장임대관리 메뉴에서 저장이 불가능합니다.");
                return;
            }
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamCmmnCntrNticArrvlDtaInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamCmmnCntrNticArrvlDtaInqireForm', {_updtId:'I'});
                inputVO[inputVO.length]={name: 'form', value: JSON.stringify(this._editData2) };    // 폼의 데이터를 컨트롤러에 보낸다.

                //console.log(inputVO);
                // 데이터를 저장 하고 난 뒤 리스트를 다시 로딩 한다.

                this.doAction('<c:url value="/oper/shed/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireForm');
                        module.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexReload();
                        module.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
        ...
        case 'btnRemoveItem':
            ...
            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 공컨장치장임대관리 메뉴에서 신청삭제가 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireForm');

                    this.doAction('<c:url value="/oper/shed/gamDeleteAssetRent.do" />', inputVO, function(module, result) {
                    ...
                    this.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamCmmnCntrNticArrvlDtaInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnSaveComment':
            ...
            this.doAction('<c:url value="/oper/shed/gamUpdateAssetRentComment.do" />', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireSearchForm');
                    module.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#cmmnCntrNticArrvlDtaInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("공컨장치장임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexGetRow(this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexRemoveRow(this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        case 'btnSaveItemDetail':
            ...
            if(this.$("#detailCmd").val()=='insert') {
                ...
                this.doAction('<c:url value="/oper/shed/gamInsertAssetRentDetail.do" />', {aaa : "rrrrrrrrrrrrrr"}, function(module, result) {
                ...
                this.doAction('<c:url value="/oper/shed/gamUpdateAssetRentDetail.do" />', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireForm');
                        module.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'btnPrmisn': // 사용승낙
            ...
            if(rows.length>=1) {
                ...
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '<c:url value="/oper/shed/popup/showAssetRentPrmisn.do"/>', opts);
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                if( confirm("승낙취소를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/shed/gamUpdateAssetRentPrmisnCancel.do" />', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireForm');
                            module.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#cmmnCntrNticArrvlDtaInqireDetailList').flexAddRow(this._editData);
            }
...
GamCmmnCntrNticArrvlDtaInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamCmmnCntrNticArrvlDtaInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCmmnCntrNticArrvlDtaInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#cmmnCntrNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamCmmnCntrNticArrvlDtaInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#cmmnCntrNticArrvlDtaInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#cmmnCntrNticArrvlDtaInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamCmmnCntrNticArrvlDtaInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamCmmnCntrNticArrvlDtaInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamCmmnCntrNticArrvlDtaInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="cmmnCntrNticArrvlDtaInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="cmmnCntrNticArrvlDtaInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <!--<div class="emdControlPanel"></div>-->
                    <form id="gamCmmnCntrNticArrvlDtaInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="cmmnCntrNticArrvlDtaInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="cmmnCntrNticArrvlDtaInqireDetailList" style="display:none"></table>
