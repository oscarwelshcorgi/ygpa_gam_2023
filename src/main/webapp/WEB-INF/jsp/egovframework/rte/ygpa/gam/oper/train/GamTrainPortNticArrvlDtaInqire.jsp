 ...
 */
function GamTrainPortNticArrvlDtaInqireModule() {}

GamTrainPortNticArrvlDtaInqireModule.prototype = new EmdModule(1000, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamTrainPortNticArrvlDtaInqireModule.prototype.loadComplete = function() {

    // 철송장임대 테이블 설정
    this.$("#trainPortNticArrvlDtaInqireList").flexigrid({
        module: this,
    ...
    // 철송장임대상세 테이블 설정
    this.$("#trainPortNticArrvlDtaInqireDetailList").flexigrid({
        module: this,
        url: '<c:url value="/oper/train/gamSelectTrainPortNticArrvlDtaInqireDetailList.do" />',
        dataType: 'json',
        colModel : [
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '<c:url value="/oper/train/gamSelectAssetRentFileList.do" />',
        dataType: 'json',
        colModel : [
					/*
    ...
    this.$("#trainPortNticArrvlDtaInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module.$('#gamTrainPortNticArrvlDtaInqireForm :input').val('');

        module.makeFormValues('#gamTrainPortNticArrvlDtaInqireForm', row);
        module._editData=module.getFormValues('#gamTrainPortNticArrvlDtaInqireForm', row);
        module._editRow=module.$('#trainPortNticArrvlDtaInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireForm');
        module.$('#trainPortNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#trainPortNticArrvlDtaInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds()[0];
    });
    ...
    this.$("#trainPortNticArrvlDtaInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds()[0];
...
GamTrainPortNticArrvlDtaInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamTrainPortNticArrvlDtaInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'searchBtn':            
            ...
            var searchOpt=this.makeFormArgs('#gamTrainPortNticArrvlDtaInqireSearchForm');
            this.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 0});
            this.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();

            break;

        // 최초신청
        case 'addAssetRentFirst':
            this.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamTrainPortNticArrvlDtaInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#trainPortNticArrvlDtaInqireDetailList").flexRemove();
            this.$("#trainPortNticArrvlDtaInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#trainPortNticArrvlDtaInqireList').selectedRows();

            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 철송장임대관리 메뉴에서 연장신청이 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/train/gamInsertTrainPortNticArrvlDtaInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireSearchForm');
                            module.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            if( this.$('#quayGroupCd').val() != 'P' ) {
                alert("해당 건은 철송장임대관리 메뉴에서 저장이 불가능합니다.");
                return;
            }
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#trainPortNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#trainPortNticArrvlDtaInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamTrainPortNticArrvlDtaInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamTrainPortNticArrvlDtaInqireForm', {_updtId:'I'});
                inputVO[inputVO.length]={name: 'form', value: JSON.stringify(this._editData2) };    // 폼의 데이터를 컨트롤러에 보낸다.

                //console.log(inputVO);
                // 데이터를 저장 하고 난 뒤 리스트를 다시 로딩 한다.

                this.doAction('<c:url value="/oper/train/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireForm');
                        module.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#trainPortNticArrvlDtaInqireDetailList').flexReload();
                        module.$('#trainPortNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
        ...
        case 'btnRemoveItem':
            ...
            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 철송장임대관리 메뉴에서 신청삭제가 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamTrainPortNticArrvlDtaInqireForm');

                    this.doAction('<c:url value="/oper/train/gamDeleteAssetRent.do" />', inputVO, function(module, result) {
                    ...
                    this.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamTrainPortNticArrvlDtaInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnSaveComment':
            ...
            this.doAction('<c:url value="/oper/train/gamUpdateAssetRentComment.do" />', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireSearchForm');
                    module.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#trainPortNticArrvlDtaInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#trainPortNticArrvlDtaInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("철송장임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#trainPortNticArrvlDtaInqireDetailList').flexGetRow(this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#trainPortNticArrvlDtaInqireDetailList').flexRemoveRow(this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        case 'btnSaveItemDetail':
            ...
            if(this.$("#detailCmd").val()=='insert') {
                ...
                this.doAction('<c:url value="/oper/train/gamInsertAssetRentDetail.do" />', {aaa : "rrrrrrrrrrrrrr"}, function(module, result) {
                ...
                this.doAction('<c:url value="/oper/train/gamUpdateAssetRentDetail.do" />', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireForm');
                        module.$('#trainPortNticArrvlDtaInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'popupEntrpsInfoInput': // 팝업을 호출한다.(철송장임대입력)
            var opts;
        ...
        case 'btnPrmisn': // 사용승낙
            ...
            if(rows.length>=1) {
                ...
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '<c:url value="/oper/train/popup/showAssetRentPrmisn.do"/>', opts);
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                if( confirm("승낙취소를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/train/gamUpdateAssetRentPrmisnCancel.do" />', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamTrainPortNticArrvlDtaInqireForm');
                            module.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#trainPortNticArrvlDtaInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#trainPortNticArrvlDtaInqireDetailList').flexAddRow(this._editData);
            }
...
GamTrainPortNticArrvlDtaInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamTrainPortNticArrvlDtaInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamTrainPortNticArrvlDtaInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#trainPortNticArrvlDtaInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamTrainPortNticArrvlDtaInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#trainPortNticArrvlDtaInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#trainPortNticArrvlDtaInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamTrainPortNticArrvlDtaInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamTrainPortNticArrvlDtaInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamTrainPortNticArrvlDtaInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="trainPortNticArrvlDtaInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                <table id="trainPortNticArrvlDtaInqireList" style="display:none" class="fillHeight"></table>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="trainPortNticArrvlDtaInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="trainPortNticArrvlDtaInqireDetailList" style="display:none"></table>
