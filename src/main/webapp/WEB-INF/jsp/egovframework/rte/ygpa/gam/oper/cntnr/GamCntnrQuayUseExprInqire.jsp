...
<%
  /**
  * @Class Name : GamCntnrQuayUseExprInqire.jsp
 ...
 */
function GamCntnrQuayUseExprInqireModule() {}

GamCntnrQuayUseExprInqireModule.prototype = new EmdModule(1100, 650);

// 페이지가 호출 되었을때 호출 되는 함수
GamCntnrQuayUseExprInqireModule.prototype.loadComplete = function() {

    // 컨테이너부두임대 테이블 설정
    this.$("#cntnrQuayUseExprInqireList").flexigrid({
        module: this,
    ...
    // 컨테이너부두임대상세 테이블 설정
    this.$("#cntnrQuayUseExprInqireDetailList").flexigrid({
        module: this,
    ...
    this.$("#assetRentFileList").flexigrid({
        module: this,
        url: '<c:url value="/oper/cntnr/gamSelectAssetRentFileList.do" />',
        dataType: 'json',
        colModel : [
					/*
    ...
    this.$("#cntnrQuayUseExprInqireList").on('onItemSelected', function(event, module, row, grid, param) {
        module.$('#cmd').val('modify');

        module.$('#gamCntnrQuayUseExprInqireForm :input').val('');

        module.makeFormValues('#gamCntnrQuayUseExprInqireForm', row);
        module._editData=module.getFormValues('#gamCntnrQuayUseExprInqireForm', row);
        module._editRow=module.$('#cntnrQuayUseExprInqireList').selectedRowIds()[0];
        ...
        var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireForm');
        module.$('#cntnrQuayUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
        module.$('#assetRentFileList').flexOptions({params:searchOpt}).flexReload();
    });

    this.$("#cntnrQuayUseExprInqireDetailList").on('onItemSelected', function(event, module, row, grid, param) {
        ...
        module._editRow=module.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds()[0];
    });

    this.$("#cntnrQuayUseExprInqireList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 1});
        module.$('#cmd').val('modify');
        module.$('#gamCntnrQuayUseExprInqireForm :input').val('');
    ...
    this.$("#cntnrQuayUseExprInqireDetailList").on('onItemDoubleClick', function(event, module, row, grid, param) {
        module.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 2});
        module.$('#gamAssetRentDetailForm :input').val('');
        module.makeFormValues('#gamAssetRentDetailForm', row);
        module._editData=module.getFormValues('#gamAssetRentDetailForm', row);
        module._editRow=module.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds()[0];
...
GamCntnrQuayUseExprInqireModule.prototype.onCalc = function() {
 ...
 * 정의 된 버튼 클릭 시
 */
 GamCntnrQuayUseExprInqireModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'addAssetRentFirst':
            this.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 1});  // 탭을 전환 한다.
            this.$('#gamCntnrQuayUseExprInqireForm').find(':input').val('');
            this.$('#gamAssetRentDetailForm').find(':input').val('');
            //this.$("#cntnrQuayUseExprInqireDetailList").flexRemove();
            this.$("#cntnrQuayUseExprInqireDetailList").flexAddData({resultList:[]}); //그리드 초기화
            this.$("#cmd").val('insert');
        ...
        case 'addAssetRentRenew':
            var rows = this.$('#cntnrQuayUseExprInqireList').selectedRows();

            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 컨테이너부두임대관리 메뉴에서 연장신청이 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length>=1) {
                ...
                if( confirm("연장신청을 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/cntnr/gamInsertCntnrQuayUseExprInqireRenew.do" />', rows[0], function(module, result) {

                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireSearchForm');
                            module.$('#cntnrQuayUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnSaveItem':
            ...
            if( this.$('#quayGroupCd').val() != 'P' ) {
                alert("해당 건은 컨테이너부두임대관리 메뉴에서 저장이 불가능합니다.");
                return;
            }
            */
            ...
            if( confirm("저장하시겠습니까?") ) {
                ...
                inputVO[inputVO.length]={name: 'updateList', value :JSON.stringify(this.$('#cntnrQuayUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'U'}])) };

                inputVO[inputVO.length]={name: 'insertList', value: JSON.stringify(this.$('#cntnrQuayUseExprInqireDetailList').selectFilterData([{col: '_updtId', filter: 'I'}])) };
                ...
                alert(inputVO);
                //var otherForm=this.getFormValues('#gamCntnrQuayUseExprInqireForm', {});  // 폼만 있을 경우

                this._editData2=this.getFormValues('#gamCntnrQuayUseExprInqireForm', {_updtId:'I'});
                inputVO[inputVO.length]={name: 'form', value: JSON.stringify(this._editData2) };    // 폼의 데이터를 컨트롤러에 보낸다.

                //console.log(inputVO);
                // 데이터를 저장 하고 난 뒤 리스트를 다시 로딩 한다.

                this.doAction('<c:url value="/oper/cntnr/gamSaveAssetRent.do" />', inputVO, function(module, result) {
                    if(result.resultCode == 0){
                        var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireForm');
                        module.$('#cntnrQuayUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        //module.$('#cntnrQuayUseExprInqireDetailList').flexReload();
                        module.$('#cntnrQuayUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
                    alert(result.resultMsg);
                });
                ...
                this.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
            }
        ...
        case 'btnRemoveItem':
            ...
            /*
            if( rows[0]['quayGroupCd'] != 'P' ) {
                alert("해당 건은 컨테이너부두임대관리 메뉴에서 신청삭제가 불가능합니다.");
                return;
            }
            */
            ...
            if(rows.length == 0) {
                ...
                if( confirm("신청삭제를 하시겠습니까?") ) {
                    ...
                    var inputVO=this.makeFormArgs('#gamCntnrQuayUseExprInqireForm');

                    this.doAction('<c:url value="/oper/cntnr/gamDeleteAssetRent.do" />', inputVO, function(module, result) {
                    ...
                    this.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 0});  // 탭을 전환 한다.
                    this.$('#gamCntnrQuayUseExprInqireForm :input').val("");
                    this.$("#cmd").val('insert');
                }
            }
        ...
        case 'btnSaveComment':
            ...
            this.doAction('<c:url value="/oper/cntnr/gamUpdateAssetRentComment.do" />', inputVO, function(module, result) {
                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireSearchForm');
                    module.$('#cntnrQuayUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnInsertItemDetail':
            if( this.$('#prtAtCode').val() == '' ) {
                ...
                this.$("#cntnrQuayUseExprInqireListTab").tabs("option", {active: 2});  // 탭을 전환 한다.
                this.$('#gamAssetRentDetailForm').find(':input').val('');
                ...
                this._editRow=this.$('#cntnrQuayUseExprInqireDetailList').flexGetData().length;
            }
        ...
        case 'btnRemoveItemDetail':
            var rows = this.$('#cntnrQuayUseExprInqireDetailList').selectedRows();

            if(rows.length == 0) {
                alert("컨테이너부두임대상세목록에서 삭제할 행을 선택하십시오.");
            } else {
                if(this.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds().length>0) {
                    for(var i=this.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds().length-1; i>=0; i--) {
                        var row=this.$('#cntnrQuayUseExprInqireDetailList').flexGetRow(this.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds()[i]);

                        if(row._updtId==undefined || row._updtId!='I') {
                            this._deleteDataList[this._deleteDataList.length]=row;  // 삽입 된 자료가 아니면 DB에 삭제를 반영한다.
                        }
                        this.$('#cntnrQuayUseExprInqireDetailList').flexRemoveRow(this.$('#cntnrQuayUseExprInqireDetailList').selectedRowIds()[i]);
                    }
                }
            }
        ...
        case 'btnSaveItemDetail':
            ...
            if(this.$("#detailCmd").val()=='insert') {
                ...
                this.doAction('<c:url value="/oper/cntnr/gamInsertAssetRentDetail.do" />', {aaa : "rrrrrrrrrrrrrr"}, function(module, result) {
                ...
                this.doAction('<c:url value="/oper/cntnr/gamUpdateAssetRentDetail.do" />', inputVO, function(module, result) {
                    if(result.resultCode=='0') {
                        var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireForm');
                        module.$('#cntnrQuayUseExprInqireDetailList').flexOptions({params:searchOpt}).flexReload();
                    }
        ...
        */
        ...
        case 'btnPrmisn': // 사용승낙
            ...
            if(rows.length>=1) {
                ...
                this.doExecuteDialog('insertAssetRentPrmisnPopup', '승낙', '<c:url value="/oper/cntnr/popup/showAssetRentPrmisn.do"/>', opts);
        ...
        case 'btnPrmisnCancel': // 승낙취소
            ...
            if(rows.length>=1) {
                if( confirm("승낙취소를 하시겠습니까?") ) {
                    this.doAction('<c:url value="/oper/cntnr/gamUpdateAssetRentPrmisnCancel.do" />', rows[0], function(module, result) {
                        if(result.resultCode=='0') {
                            var searchOpt=module.makeFormArgs('#gamCntnrQuayUseExprInqireForm');
                            module.$('#cntnrQuayUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
                        }
        ...
        case 'btnRentDetailApply': //임대상세적용
            ...
            if(this._editRow!=null) {  // 이전에 _updtId 로 선택 한 것을 _editRow 로 변경 2014-03-14.001
                ...
                this.$('#cntnrQuayUseExprInqireDetailList').flexUpdateRow(this._editRow, this._editData);
                this._editRow=null;    // 편집 저장 하였으므로 로우 편집을 종료 한다.
            }
            else {
                this.$('#cntnrQuayUseExprInqireDetailList').flexAddRow(this._editData);
            }
...
GamCntnrQuayUseExprInqireModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamCntnrQuayUseExprInqireModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCntnrQuayUseExprInqireSearchForm');
    //this.showAlert(searchOpt);
    this.$('#cntnrQuayUseExprInqireList').flexOptions({params:searchOpt}).flexReload();
};

GamCntnrQuayUseExprInqireModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
        var row = this.$('#cntnrQuayUseExprInqireList').selectedRows();
        if(row.length==0) {
            this.$('#cmd').val('insert');
        }
        else {
        ...
        break;
    case 'tabs3':
        var row = this.$('#cntnrQuayUseExprInqireDetailList').selectedRows();
        if(row.length==0) {
            this.$('#detailCmd').val('insert');
        }
        else {
...
GamCntnrQuayUseExprInqireModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamCntnrQuayUseExprInqireModule();
...
<div class="window_main">
    ...
    <div id="searchViewStack" class="emdPanel">
        <div class="viewPanel">
            <form id="gamCntnrQuayUseExprInqireSearchForm">
    ...
    <div class="emdPanel fillHeight">
        <div id="cntnrQuayUseExprInqireListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange">
            <ul>
                <li><a href="#tabs1" class="emdTab">컨테이너부두임대(만기도래) 목록</a></li>
                <li><a href="#tabs2" class="emdTab">컨테이너부두임대(만기도래) 내역</a></li>
            ...
            <div id="tabs2" class="emdTabPage" style="overflow: scroll;">
                <div class="emdControlPanel"></div>
                    <form id="gamCntnrQuayUseExprInqireForm">
                ...
                <div style="vertical-align: bottom; text-align: right;">
                 ...
                 <!-- <table id="cntnrQuayUseExprInqireDetailList" style="display:none" class="fillHeight"></table> -->
                 <table id="cntnrQuayUseExprInqireDetailList" style="display:none"></table>
