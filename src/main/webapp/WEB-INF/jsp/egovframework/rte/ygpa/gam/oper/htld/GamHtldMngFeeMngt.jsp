 ...
 */
function GamHtldMngtFeeMngtModule() {}

GamHtldMngtFeeMngtModule.prototype = new EmdModule(1070, 600);

// 페이지가 호출 되었을때 호출 되는 함수
GamHtldMngtFeeMngtModule.prototype.loadComplete = function(params) {

    // 테이블 설정 //
    this.$("#assetRentFeeList").flexigrid({
        module: this,
        url: '/oper/htld/gamSelectHtldMngFeeMngtList.do',
        dataType: 'json',
        colModel : [
				{display:'상태', name:'state',width:30, sortable:false,align:'center'},
                {display:'구역', name:'rentArea',width:82, sortable:false,align:'center'},
				{display:'고지월', name:'nticCnt',width:50, sortable:false,align:'center'},
				...
				{display:'사용면적', name:'area',width:80, sortable:false,align:'right', displayFormat: 'input-number'},
				{display:'면적단가', name:'areaFee',width:80, sortable:false,align:'right', displayFormat: 'input-number'},
				{display:'관리비', name:'fee',width:80, sortable:false,align:'right', displayFormat: 'input-number'},
    ...
    this.$("#assetRentFeeList").on('onCellEdited', function(event, module, row, rid, cid) {
        ...
        if(cid=="areaFee") {
			...
			var areaFee=Number(row['areaFee']);
            if(row._updtId!="I") {
            	row._updtId="U";
            	row.state="*";
            }

            var fee=Math.floor(areaFee*area*0.01)*10;
			row['fee']=fee;
            if(row['vatYn']=='2' || row['vatYn']=='Y') {
                vat=Math.floor(fee*0.01)*10;
            }
...
/*     this.$("#nticListGrid").flexigrid({
    ...
    if(params!=null) {
    	if(params.action=="selectRentFee") {
    	    ...
    	    this.$('#nticYear').val(EMD.util.getDate().getFullYear());
    	    this.$('#nticMon').val(EMD.util.getDate().getMonth()+1);
...
GamHtldMngtFeeMngtModule.prototype.setEvents = function() {
...
GamHtldMngtFeeMngtModule.prototype.setButtonStatus = function() {
...
GamHtldMngtFeeMngtModule.prototype.makeRowData = function(item) {
...
<%--
 정의 된 버튼 클릭 시
--%>
 GamHtldMngtFeeMngtModule.prototype.onButtonClick = function(buttonId) {
...
GamHtldMngtFeeMngtModule.prototype.onCalc = function() {
	var sum = {
			sumCnt:0,
			sumFee:0,
			sumVat:0,
...
<%--
GamHtldMngtFeeMngtModule.prototype.saveNticList = function() {
	var nticList = {};
	this.onCalc();
...
GamHtldMngtFeeMngtModule.prototype.clearNticList = function() {
	var nticYn='N';
	this.$('#assetRentFeeList')[0].dgrid.forEachRow(function(id) {
...
--%>
GamHtldMngtFeeMngtModule.prototype.openNticIssuePopup = function() {
    if(this.$('#assetRentFeeList').selectedRowCount()>0) {
...
GamHtldMngtFeeMngtModule.prototype.onSubmit = function() {
    //this.showAlert(this.$('#prtCode').val()+'을(를) 조회 하였습니다');

    this.loadData();
};

GamHtldMngtFeeMngtModule.prototype.loadData = function() {
...
GamHtldMngtFeeMngtModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	if(newTabId=='tabs2') {
...
GamHtldMngtFeeMngtModule.prototype.onTabChange = function(newTabId, oldTabId) {
    switch(newTabId) {
    case 'tabs1':
        break;
    case 'tabs2':
...
GamHtldMngtFeeMngtModule.prototype.loadDetail = function() {
    	var row = this.$('#assetRentFeeList').selectedRows()[0];
...
%>
GamHtldMngtFeeMngtModule.prototype.onClosePopup = function(popupId, msg, value) {
    switch (popupId) {
...
var module_instance = new GamHtldMngtFeeMngtModule();
</script>
<!-- 아래는 고정 -->
            ...
            <form id="gamAssetRentFeeSearchForm">
                ...
                <table style="width:100%;" class="searchPanel">
                        ...
                        <tr>
                            <th>고지 년월</th>
                            <td colspan="3">
                            	<input id="nticYear" type="text" class="emdcal" size="4"> ~
                            	<input id="nticMon" type="text" class="emdcal" size="2">
                            </td>
                        </tr>
                </table>
            </form>
