...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.loadComplete = function(params) {

    // 테이블 설정 //
    this.$("#cmmnCntrRentFeePaySttusMngtList").flexigrid({
        module: this,
        url: '/oper/shed/gamSelectCmmnCntrRentFeePaySttusMngtList.do',
        dataType: 'json',
        colModel : [
...
//					{display:'요금종류', name:'chrgeKnd',width:55, sortable:false,align:'center'},
        ...
        preProcess: function(module,data) {
        	...
        	module.makeDivValues('#cmmnCntrRentFeePaySttusMngtListSum', data);
            */
            return data;
        }
    });
    ...
    */
    ...
    /* 연체내역 목록 */
    this.$("#cmmnCntrRentFeePaySttusArrrgList").flexigrid({
        module: this,
        url: '/oper/shed/selectCmmnCntrRentFeePaySttusMngtDlyList.do',
        dataType: 'json',
        colModel : [
        ...
        preProcess: function(module,data) {
    		if (data.resultCode == "0") {
    			module.makeDivValues('#cmmnCntrRentFeePaySttusMngtListForm',data.resultDlyInfo);	// 리스트 값을 채운다

    			module.makeDivValues('#cmmnCntrRentFeePaySttusMngtSum', data.resultSummary); // 결과값을 채운다.
 ...
 GamCmmnCntrRentFeePaySttusMngtModule.prototype.onButtonClick = function(buttonId) {
    ...
    switch(buttonId) {
        ...
        case 'btnUpdatePayDtls':	// 납부 현황 새로 고침
            this.doAction('/oper/shed/updateCmmnCntrRentFeePaySttusMngtList.do', null, function(module, result) {

                if(result.resultCode=='0') {
                    var searchOpt=module.makeFormArgs('#gamCmmnCntrRentFeePaySttusMngtSearchForm');
                    module.$('#cmmnCntrRentFeePaySttusMngtList').flexOptions({params:searchOpt}).flexReload();
                }
        ...
        case 'btnNticArrrg':
            this.doExecuteDialog('nticArrrgPopup', '연체 일괄 고지', '/oper/shed/showNticArrrgPopup.do', opts);
        	break;
        ...
        case 'btnNticArrrgCancelPk':
        	var rows = this.$('#cmmnCntrRentFeePaySttusArrrgList').selectedRows();
			if(rows.length==0) {
        ...
        case 'btnNticIssuePrint':
        	var rows = this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows();
        	if(rows.length==0) {
            ...
            this.printPayNotice('/oper/shed/printCmmnCntrRentFeePayNotice.do', row);
        	break;
        case 'btnNticIssuePrint2':
            ...
            this.printPayNotice('/oper/shed/printCmmnCntrRentFeePayNotice2.do', row);
        	break;
        case 'btnCmmnCntrRentFeePaySttusMngtListExcelDownload':	// 엑셀 다운로드
    		this.$('#cmmnCntrRentFeePaySttusMngtList').flexExcelDown('/oper/shed/selectCmmnCntrRentFeePaySttusMngtListExcel.do');
    		break;
    }
};

GamCmmnCntrRentFeePaySttusMngtModule.prototype.nticArrrgSingle = function() {
	var rows = this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows();
	if(rows.length==0) {
		alert('연체 고지 할 항목을 선택 하세요.');
		return;
...
//	}
	...
	if(this.resultDetail.arrrgNo!=null) {	// 기 연체 고지된 건이 있는지
	 	this.doAction('/oper/shed/updateNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/shed/printCmmnCntrRentFeePayNotice.do', row);
				}
			} else {
				alert(result.resultMsg);
			}
	 	...
	 	this.doAction('/oper/shed/insertNticArrrg.do', arrrgDetail, function(module, result) {
			if (result.resultCode == "0") {
				if(confirm('연체 고지 되었습니다. 바로 납부 고지서를 출력 하시겠습니까?')) {
					module.printPayNotice('/oper/shed/printCmmnCntrRentFeePayNotice.do', row);
				}
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.receiveFeeSingle = function() {
    if(this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRowCount()>0) {

        var rows = this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows()[0];

     	this.doAction('/oper/shed/checkOcrResult.do', rows, function(module, result) {
    		if (result.resultCode == "0") {
    	    	...
    	    	module.doExecuteDialog('feePayPopup', '수납 처리', '/oper/shed/showFeePayPopup.do', opts);
    		} else {
    			alert(result.resultMsg);
    		}
    	});
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.nticArrrgCancelAll = function() {
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/shed/cancelNticArrrg.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.nticArrrgCancelPk = function(ntic) {
	var row=this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows()[0];
	...
	var arrrgDetail = [
 	...
 	this.doAction('/oper/shed/cancelNticArrrgPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.nticArrrgPrintCancelPk = function(ntic) {
	var arrrgDetail = [
	               { name: 'prtAtCode', value: ntic.prtAtCode},
	               { name: 'fiscalYr', value: ntic.fiscalYr },
	               { name: 'billNo', value: ntic.billNo },
 	...
 	this.doAction('/oper/shed/cancelNticArrrgPrintPk.do', arrrgDetail, function(module, result) {
		if (result.resultCode == "0") {
			alert('고지 취소 되었습니다.');
			module.loadData();
		} else {
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.loadData = function() {
    var searchOpt=this.makeFormArgs('#gamCmmnCntrRentFeePaySttusMngtSearchForm');
    this.$("#cmmnCntrRentFeePaySttusMngtListTab").tabs("option", {active: 0});    // 탭을 전환 한다.
    this.$('#cmmnCntrRentFeePaySttusMngtList').flexOptions({params:searchOpt}).flexReload();
};

GamCmmnCntrRentFeePaySttusMngtModule.prototype.loadDetailPage = function() {
	var row = this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows()[0];
	...
	var nticDetail = [
	 ...
	 this.doAction('/oper/shed/selectCmmnCntrRentFeePaySttusMngtDetail.do', nticDetail, function(module, result) {
		if (result.resultCode == "0") {
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.calcDlyDueDate = function() {
	// 납부기한 계산
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.loadArrrgPage = function() {
	var row = this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRows()[0];
	var searchOpt = [];
    ...
    this.$('#cmmnCntrRentFeePaySttusArrrgList').flexOptions({params:searchOpt}).flexReload();
	/*
	var dlyList = [
	               { name: 'prtAtCode', value: row.prtAtCode},
	               { name: 'chrgeKnd', value: row.chrgeKnd },
	...
	this.doAction('/oper/shed/selectCmmnCntrRentFeePaySttusMngtDlyList.do', dlyList, function(module, result) {
		if (result.resultCode == "0") {
			module.makeMultiDivValues('#cmmnCntrRentFeePaySttusMngtListForm',result.resultList , function(row) {
			} );	// 리스트 값을 채운다

			module.makeDivValues('#cmmnCntrRentFeePaySttusMngtSum', result.resultSummary); // 결과값을 채운다.
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.getMonthInterval = function (time1,time2,gubun){
	/* 날짜 계산 */
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.getDayInterval = function (fdate,tdate){
...
GamCmmnCntrRentFeePaySttusMngtModule.prototype.onTabChangeBefore = function(newTabId, oldTabId) {
	//change**
	if(newTabId=='tabs2' || newTabId=='tabs3') {
		if(this.$('#cmmnCntrRentFeePaySttusMngtList').selectedRowCount()!=1) {
...
<div class="window_main">
    ...
    <div class="emdPanel fillHeight">
        <div id="cmmnCntrRentFeePaySttusMngtListTab" class="emdTabPanel fillHeight" data-onchange="onTabChange" data-onchange-before="onTabChangeBefore">
            <ul>
                <li><a href="#tabs1" class="emdTab">항만관련부지임대사용료납부현황 목록</a></li>
                <li><a href="#tabs2" class="emdTab">항만관련부지임대사용료납부현황 상세</a></li>
                <!-- change** -->
                <li><a href="#tabs3" class="emdTab">항만관련부지임대사용료연체현황 목록</a></li>
            </ul>
            ...
            <div id="tabs1" class="emdTabPage fillHeight" style="overflow: hidden;" data-onactivate="onShowTab1Activate">
                ...
                <div id="cmmnCntrRentFeePaySttusMngtListSum" class="emdControlPanel">
					<form id="summaryForm">
                		...
                		<tr>
							...
							<td><button id="btnCmmnCntrRentFeePaySttusMngtListExcelDownload">엑셀</button></td>
			...
			<div id="tabs3" class="emdTabPage" style="overflow: scroll;">
                ...
                <!-- <div class="emdControlPanel"><button id="btnSaveItem">저장</button><button id="btnCancelItem">취소</button><button id="btnRemoveItem">삭제</button></div>  -->
                    <form id="cmmnCntrRentFeePaySttusMngtListForm">
                    	<div>
                    ...
                    <h2>연체 고지 목록</h2>
                    <table id="cmmnCntrRentFeePaySttusArrrgList" style="display:none" class="fillHeight"></table>
                    <table style="width:100%; margin-bottom: 2px" id="cmmnCntrRentFeePaySttusMngtSum" class="summaryPanel">
						<tr>
