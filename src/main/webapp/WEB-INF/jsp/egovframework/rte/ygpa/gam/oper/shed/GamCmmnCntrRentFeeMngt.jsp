...
//                 	rows['payTmlmt']=this.$('#payTmlmt').val();
        ...
        case 'btnNoticeAdit2':
            ...
            if(rows.length>=1) {
                this.doExecuteDialog('insertLevReqestAdit', '사용료 추가', '<c:url value="/oper/shed/popupLevReqestAdit.do"/>', rows[0]);
            } else {
                alert("목록에서 선택하십시오.");
            }
        ...
        case 'btnCmmnCntrRentFeeMngtListExcelDownload':	// 엑셀 다운로드
        	this.$('#assetRentFeeList').flexExcelDown('<c:url value="/oper/gnrl/selectCmmnCntrRentFeeMngtListExcel.do"/>');
            break;
