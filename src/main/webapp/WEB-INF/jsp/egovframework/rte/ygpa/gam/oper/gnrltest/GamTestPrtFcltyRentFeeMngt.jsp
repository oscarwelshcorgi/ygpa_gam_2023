...
GamAssetRentFeeMngtModule.prototype.loadComplete = function(params) {
    ...
    this.$("#assetRentFeeList").flexigrid({
        module: this,
        url: '/oper/gnrltest/gamSelectPrtFcltyRentFeeMngtList.do',
        dataType: 'json',
        colModel : [
...
GamAssetRentFeeMngtModule.prototype.execSave = function() {
    ...
    if( confirm("선택한 건을 상계 하시겠습니까?") ) {
        this.doAction('/oper/gnrltest/updatePrtFcltyRentFeeSaveSingle.do', row, function(module, result) {
...
GamAssetRentFeeMngtModule.prototype.cancelSave = function() {
	...
	if( confirm("선택한 건을 취소 하시겠습니까?") ) {
	    this.doAction('/oper/gnrltest/cancelPrtFcltyRentFeeSaveSingle.do', row, function(module, result) {
	        if(result.resultCode=='0') {
	            var row = module.$('#assetRentFeeList').selectedRows()[0];
	        	row['save']='';
	    		row['saveNm']="";
    ...
    switch(buttonId) {
            ...
            if(this.$('#assetRentFeeList').selectedRowCount()>0) {
                ...
                */
                ...
                if( confirm("선택한 건을 고지 하시겠습니까?") ) {
                    ...
                    this.doAction('/oper/gnrltest/insertPrtFcltyRentFeeNticSingle.do', rows, function(module, result) {
        ...
        case 'btnCancelNticIssue2':
            if(this.$('#assetRentFeeList').selectedRowCount()>0) {
                ...
                if( confirm("선택한 건의 고지를 취소 하시겠습니까?") ) {
                    ...
                    this.doAction('/oper/gnrltest/cancelPrtFcltyRentFeeNticSingle.do', rows, function(module, result) {
        ...
        case 'btnNticIssuePrint2':	// 고지서 출력
            if(this.$('#assetRentFeeList').selectedRowCount()>0) {
                ...
                this.printPayNotice('/oper/gnrltest/printRentFeeNotice.do', rows);
            } else {
        ...
        case 'btnTaxPrint2':	// 계산서 출력
            if(this.$('#assetRentFeeList').selectedRowCount()>0) {
                ...
                this.printTaxNotice('/oper/gnrltest/printPrtFcltyRentFeeTaxNotice.do', rows);
            } else {
            	alert("목록에서 고지서를 출력 할 항목을 선택하십시오.");
            	return;
            }
        ...
        case 'btnNoticeAdit2':
            ...
            if(rows.length>=1) {
                this.doExecuteDialog('insertLevReqestAdit', '사용료 추가', '/oper/gnrltest/popupLevReqestAdit.do', rows[0]);
            } else {
                alert("목록에서 선택하십시오.");
            }
            var searchOpt=this.makeFormArgs('#gamAssetRentFeeSearchForm');
        ...
        case 'btnSaveRmk':	// 비고 저장
        	var ntcUpdate = this.makeFormArgs('#gamAssetRentFeeForm');
    	   	 	this.doAction('/oper/gnrltest/updatePrtFcltyRentFeeMngtListDetail.do', ntcUpdate, function(module, result) {
    				if (result.resultCode == "0") {
    					module.loadData();
    				} else {
    				}
        ...
        case 'btnPrtFcltyRentFeeMngtListExcelDownload':	// 엑셀 다운로드
        	this.$('#assetRentFeeList').flexExcelDown('/oper/gnrltest/selectPrtFcltyRentFeeMngtListExcel.do');
            	break;
...
GamAssetRentFeeMngtModule.prototype.loadDetail = function() {
	...
	this.doAction('/oper/gnrltest/gamSelectPrtFcltyRentFeeMngtListDetail.do', nticDetail, function(module, result) {
		if (result.resultCode == "0") {
			/*
