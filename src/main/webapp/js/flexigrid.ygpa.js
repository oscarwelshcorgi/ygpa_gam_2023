 ...
 * 그리드 이벤트 목록
 *
 * 이벤트명 : onLoadDataComplete	- 데이터가 정상 적으로 수신 되었음
 * @param module	- 모듈
 * @param data		- 데이터
 * @param grid		- 그리드 클래스
 * @param params - 그리드 파라메터
 *
 * 이벤트명 : onSelectChanged	- 그리드의 선택이 변경 되었음
 * @param module	- 모듈
 * @param row		- 선택된 행 데이터 오브젝트
 * @param grid		- 그리드 클래스
 * @param params - 그리드 파라메터
 *
 * 이벤트명 : onButtonClicked	- 그리드 안에 버튼이 클릭 되었음
 * @param module		- 모듈
 * @param buttonId	- 버튼 아이디
 * @param row			- 버튼이 있는 행 데이터 오브젝트
 * @param grid			- 그리드 클래스
 * @param params 	- 그리드 파라메터
 *
 * 이벤트명 : onCheckBoxChanged	- 체크박스가 변경 되었음
 * @param module	- 모듈
 * @param checkId	- 체크박스 아이디
 * @param row		- 버튼이 있는 행 데이터 오브젝트
 * @param grid		- 그리드 클래스
 * @param params - 그리드 파라메터
 *
 * 이벤트명 : onItemSelected	- 그리드의 행이 선택 되었음 (그리드 선택과 같음)
 * @param module	- 모듈
 * @param data		- 데이터
 * @param grid		- 그리드 클래스
 * @param params - 그리드 파라메터
 *
 * 이벤트명 : onItemDoubleClick	- 그리드의 행을 더블 클릭 하였음
 * @param module	- 모듈
 * @param data		- 데이터
 * @param grid		- 그리드 클래스
 * @param params - 그리드 파라메터
...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			module: null,	// module add by eunsungj
			...
			resizable: false, //allow table resizing (denied)
			nohresize: false,
		...
		p.newp=false;
		if(p.rp==null || p.rp<1) p.rp=30;	//default page row setting
		p.useRp=false;
		p.usepager=false;
//		if(p.height=='auto') p.height=$(t).height();
		...
		g = {
			...
			rePosDrag: function () {
				...
				if (g.cDiv.scrollLeft > 0) cdleft -= Math.floor(p.cgwidth / 2);
				$(g.cDrag).css({
			...
			fixHeight: function (newH) {
				newH = false;
				if (!newH) newH = $(g.cDiv).height();
				var hdHeight = $(g.hDiv).height();
				if($(g.hTable).width()>$(g.cDiv).width()) {	// sub horz scroll bar size
					newH-=20;
				}
				$('div', g.cDrag).each(
					function () {
...
//					data = $.extend({rows: [], pageIndex: 0, totalCount: 0}, data.resultList);
				g.loading = false;
				if (p.preProcess) {
...
//					p.pageIndex = data.searchOption.pageIndex;
				...
				g.addrows(data.resultList, tbody);	// 2014-03-18 추가
...
//		        g.reflowScrollBarContent(g, t);
			...
			appendData: function (data) { //parse data
//				var btsize=$(t).height();
//				data = $.extend({rows: [], pageIndex: 0, totalCount: 0}, data.resultList);
				g.loading = false;
			if (p.preProcess) {
...
//				event.data.result[idx][$(this).attr('name').substring(0, spl)]=selectedValue;
			...
			p.data=p.data.concat(data.resultList);	// 데이터를 합친다.
			tbody = null;
			data = null;
			i = null;
			if (p.onSuccess) {
                                ...
                                if(typeof p.colModel[idx].displayFormat!='undefined') {
                                	...
                                	case 'button':
                                		if(p.colModel[idx].displayOption==null)
                                			...
                                			if(typeof p.colModel[idx].displayOption.className!='undefined') iHTML=iHTML+' class="'+p.colModel[idx].displayOption.className+'"';
                                			iHTML = iHTML+'>';
                                			if(typeof p.colModel[idx].displayOption.label!='undefined') iHTML=iHTML+p.colModel[idx].displayOption.label;
                                			iHTML = iHTML+'</BUTTON>';
                                		}
                                		break;
                                	case 'checkbox':
                                	...
                                	case 'date':
                                		break;
//                                	case 'button':
//                                		iHTML = '<button class="' + p.colModel[idx].name + '">' + iHTML + '</button>';
//                                		break;
                                	}

                                    if (typeof iHTML != 'undefined') {
                                        iHTML = (iHTML !== null) ? iHTML : ''; //null-check for Opera-browser
                                    } else {
                                        iHTML = '';
                                    }
                                    if(iHTML=="null") iHTML="";
			...
			populate: function () { //get latest data
				if (this.loading) {
					if(this.pageIndex==1) alert('데이터를 로딩중입니다. 장시간 연결 되지 않을 경우 창을 닫고 다시 실행 하시기 바랍니다.');
					return true;
				}
				if (p.onSubmit) {
					var gh = p.onSubmit();
...
//				EMD.util.showProgress();
				...
				var param = [{
					...
					name: 'pageUnit',
					value: p.rp	// 2014-03-18 수정 오토 스크롤 페이지로 수정
				}, {
					name: 'recordCountPerPage',
					value: p.rp	// 2014-03-18	다시 수정 오토 스크롤
				}];
				if (p.params.length) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if(data.resultCode!=null && data.resultCode!=0) {
							...
							// 자동 스크롤 수정
							if(p.pageIndex==1) g.addData(data);
							else g.appendData(data);
...
//							EMD.util.hideProgress();
			...
			addCellProp: function () {
				$('tbody tr td', g.cDiv).each(function () {
					if($(this).hasClass('_D_')) return true;
...
//				});
				$('tbody tr', g.bDiv).on('click', function (e) {
					...
					/*var row=[];
					$.each(this.cells, function (c, cell) {
						var col = cell.abbr;
...
//						$(this).closest('table').find('.trSelected').each(function() {
...
//					$(this).closest('.flexigrid').trigger('onItemDoubleClick', [this, g, p]);
/*					var row={};
					...
					$('thead tr:first th', g.hDiv).each(function () {
						...
						var val = cells[this.cellIndex].firstChild.innerHTML;
						if (val == '&nbsp;') val = '';      // Trim the content
//		        		        var idx = cell.cellIndex;
						if(col==null || col=='') {
							alert('column error : column value is '+val);
						}

						row[col]=val;
//						var obj = jQuery.parseJSON('{'+col+': "'+val+'"}');
//        		        $.extend(row, obj);
					});*/
					$(this).addClass('trSelected');
...
//					}
			...
			pager: 0,
			contentSizeReset: function(g, t) {
		        g.resetScrollBarValue(g, t);
		        g.sizeScrollbar(g, t);
		        g.reflowScrollBarContent(g, t);
			},
		      //size g.scrollbar and handle proportionally to scroll distance
			sizeScrollbar: function (g, t) {
				if(g==null || g.scrollbar==null) return;
...
//		        var remainder = th - ch;
		        ...
		        g.scrollbar.find( ".ui-slider-handle" ).css({
		          height: handleSize,
		          "margin-top": 0
		        });
//		        g.scrollbar.slider("value", 100);
		      ...
		      resetScrollBarValue: function (g, t) {
		    	  ...
		    	  var leftVal = $(t).css( "margin-top" ) === "auto" ? 0 :
		      ...
		      addScrollBar: function(g, t) {
		  		$(g.cDiv).append('<div class="scroll-bar-wrap ui-widget-content ui-corner-all">\
					    <div class="scroll-bar"></div>\
					    </div>');
//		  			$(g.cDiv).append('<div class="scroll-bar"></div>');

		    	  g.scrollbar= $(g.cDiv).find(".scroll-bar" ).slider({
			  	    	min: 0,
			  	    	max: 100,
			  	    	value: 100,	// reverse
...
//			        g.scrollbar.width( g.handleHelper.width() );
			      })
			      .mouseup(function() {
//			        g.scrollbar.width( "100%" );
			      })
			      ...
			      //init g.scrollbar size
			      g.resetScrollBarValue(g, t);
			      g.sizeScrollbar(g, t);
			      setTimeout( g.sizeScrollbar, 10, {g:g, t:t} );	//safari wants a timeout
			      g.reflowScrollBarContent(g, t);
...
//						th.align = cm.align;
		...
		g.cDiv = document.createElement('div'); //create contents container
...
//		$(t).before(g.hDiv);
		...
		$(g.cDiv).resize(function() {
			if($(g.cDiv).width()>$(t).width()) $(g.cDiv).width($(t).width());
		});
//		$(t).before(g.bDiv);
		$(g.gDiv).css({
...
//			$('table', g.bDiv).addClass('fillHeight');
		}
...
*/
	...
	$.fn.flexEmptyData = function (p) {
		return this.each(function () {
			this.p.pageIndex=1;
			this.p.pages=null;
			if (this.grid) this.grid.addData({resultList: []});
		});
	};
	/**
	 * 그리드의 설정을 변경 한다.
	...
	/**
	 * 그리드에 데이터를 추가한다.
	 * @param data		- 데이터 목록을 가지고 있는 오브젝트 (data.resultList)
	 */
...
//
//		if(selector.data==null) selector.data=[];
...
//		return selector.data.length;
	...
	$.fn.flexRemoveRow = function(rowIndex) {
		...
		if (grid) {
			grid.addData(dl);
//			grid.contentSizeReset(grid, this);
		}
//		var dl={};
//			data = this.p.data;
//			data.length--;
...
//			dl.resultList=data;
//		if (this.grid) this.grid.addData(dl);
	};
	/**
	 * 모든 선택을 취소 한다.
	 */
	...
	$.fn.clearSelect = function (p) { //no select plugin by me :-)
		var arRow = [];
...
//			});
	...
	$.fn.unSelectedRows = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
		var arReturn = [];
//		var arRow = [];
...
//			});
	...
	$.fn.selectedRowIds = function (p) { // Returns the selected rows as an array, taken and adapted from http://stackoverflow.com/questions/11868404/flexigrid-get-selected-row-columns-values
		var arRow = [];
		var selector = $(this.selector + ' .trSelected');
	...
	$.fn.flexGetMergeData = function(p) {
		...
		$.each(selector.data, function() {
			if($(this)._updt==null || $(this)._updt=="") return;
...
//		var arRow = [];
		...
		$.each(selector.data, function (i, row) {
			...
			$.each(filter, function() {
				if(typeof row[this.col]!='undefined' && row[this.col]==this.filter ) filtered=true;
				return filtered==false;
			});
			if(filtered) arReturn.push(row);
		});
		return arReturn;
	};
...
//			$("#__templateXls").empty();
			...
			if(fileName==null) fileName="조회결과.xls";
			param[param.length]={name: 'header', value :JSON.stringify(header) };
			param[param.length]={name: 'fileName', value: fileName };
			$.fileDownload(url, {data:param, httpMethod:"POST"});	// 헤더를 전송 해야 해서 컨트롤러를 따로 생성 해야 함
			// 아래는 테이블로 부터 직접 엑셀 파일을 추출하는 방식 (대신 작은 파일 만 생성이 가능 하다.)
//			$.ajax({
//				type: p.method,
//				url: url,
//				data: param,
...
/*					if(data.resultCode!=null && data.resultCode!=0) {
						...
						var g=this.tableModule.grid;
						if (p.preProcess) {
							data = p.preProcess(p.module, data);
						}
						var tbl = document.createElement('table');
						var thead = $('thead', g.hDiv).clone();
						$(thead).removeAttr('abbr');
						$(thead).removeAttr('axis');
						$(tbl).append(thead);
						var tbody = document.createElement('tbody');
...
//		                                		break;
						...
						$(tbl).append(tbody);
						$(tbl).css("border", "solid black");
						$("thead th", tbl).css("background-color", "#6EB6FF");

						$("#__templateXls").append(tbl);

					  var uri = 'data:application/vnd.ms-excel;base64,'
						    , template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{table}</table></body></html>'
						    , base64 = function(s) { return window.btoa(unescape(encodeURIComponent(s))); }
						    , format = function(s, c) { return s.replace(/{(\w+)}/g, function(m, p) { return c[p]; }); };
						var ctx = {worksheet: 'Sheet1' || 'Worksheet', table: $("#__templateXls table").html()};
					    window.location.href = uri + base64(format(template, ctx));

						$(this.tableModule).trigger('onLoadXlsComplete', [this.paramData.module, p.data, g, p]);
						$("#__templateXls").empty();
					}*/
//				},
//				error: function (XMLHttpRequest, textStatus, errorThrown) {
