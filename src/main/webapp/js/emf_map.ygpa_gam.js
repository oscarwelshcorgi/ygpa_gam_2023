...
var EMD = function($, window, document, undefined) {
  ...
  }, context_root:"/", gis_engine_url:"http://lfitsvr.iptime.org:8080/G2DataService/GService?", serverUrl:"", vworldKey:"", window_seq:0, win_x:80, win_y:40, drag_handle:null, dragging:false, drag_ui:null, userinfo:null, popupCaller:null, popupId:null, popupParams:null, map:null, gyhmap:null, wfs:null, lotarea:null, lotareaProtocol:null, panel:null, draw:null, edit:null, edit_win:null, del:null, info:null, save:null, saveStrategy:null, transaction:{type:null, callbackWindowId:null}, map_panel:null, 
  userLayer:{landCode:null, prtFclty:null, gisAssetsCd:null}, protocols:{prtFclty:null, gisAssetsCd:null}, selectControl:null, dragFeature:null, maxBounds:new OpenLayers.Bounds(-3E4, -6E4, 494288, 988576), maxRes:0.0439453125, maxZoom:14, module_instance:[], init:{frame_breaker:function() {
    ...
    EMD.lotarea.events.register("featureselected", null, function(e) {
      ...
      EMD.gis.getBupjungDongNm(e.feature.attributes.PNU.substring(0, 10), {feature:feature}, function(data) {
        ...
        info += "</div>";
        var popup = new OpenLayers.Popup.FramedCloud("popup", data.feature.geometry.bounds.getCenterLonLat(), null, info, null, true);
        ...
        $("#addLotcodeToAsset").on("click", {feature:feature}, function(event) {
          EMD.util.create_window("\uc790\uc0b0\uc815\ubcf4\ucd94\uac00", EMD.context_root + "code/assets/gamAssetCodeMngt.do", null, {action:"addLotcodeFeature", feature:event.data.feature.id});
        });
      });
    });
    ...
    EMD.selectControl = new OpenLayers.Control.SelectFeature([EMD.lotarea, EMD.userLayer.gisAssetsCd], {clickout:true, toggle:false, multiple:true, hover:false, toggleKey:"ctrlKey", multipleKey:"shiftKey"});
    ...
    EMD.userLayer.gisAssetsCd.events.register("featureselected", null, function(e) {
      var feature = e.feature;
      var popup = new OpenLayers.Popup.FramedCloud("popup", OpenLayers.LonLat.fromString(feature.geometry.toShortString()), null, "<div style='font-size:.8em'>ID: " + feature.id + "<br>\uc2dc\uc124\ucf54\ub4dc: " + feature.attributes.gisAssetsPrtAtCode + "<br> \uc2dc\uc124\uba85 : " + feature.attributes.gisAssetsNm + "</div>", null, true);
      feature.popup = popup;
      EMD.map.addPopup(popup);
    });
    ...
    $.ajax({url:EMD.context_root + "cmmn/selectBupJungDongCodeNm.do", type:"POST", userData:{callBack:callBackFunc, data:dataObj}, dataType:"json", contentType:"application/x-www-form-urlencoded; charset=UTF-8", data:[{name:"cmd", value:bjdCode}], success:function(data) {
  ...
  $.addFlex = function(t, p) {
    ...
    var g = {scrollbar:null, handleHelper:null, hset:{}, rePosDrag:function() {
      ...
      $(t).find("select").change({"p":p, "g":g}, function(event) {
        ...
        event.data.p.data[idx][$(this).attr("name").substring(0, spl)] = $(this).val();
        $(this).closest("table").trigger("onSelectChanged", [event.data.p.module, event.data.p.data[idx], event.data.g, event.data.p]);
      });
      $(t).find("button").on("click", {"p":p, "g":g}, function(event) {
        event.preventDefault();
        var spl = $(this).attr("name").lastIndexOf("_");
        var idx = $(this).attr("name").substring(spl + 1) * 1;
        $(this).closest("table").trigger("onButtonClicked", [event.data.p.module, $(this).attr("name").substring(0, spl), event.data.p.data[idx], event.data.g, event.data.p]);
      });
      $(t).find("input:checkbox").on("change", {"p":p, "g":g}, function(event) {
        ...
        event.data.p.data[idx][$(this).attr("name").substring(0, spl)] = selectedValue;
        $(this).closest("table").trigger("onCheckBoxChanged", [event.data.p.module, $(this).attr("name").substring(0, spl), event.data.p.data[idx], event.data.g, event.data.p]);
      });
      ...
      g.sizeScrollbar(g, t);
      $(t).on("mousewheel DOMMouseScroll", {scrollbar:g.scrollbar}, function(e) {
        var o = e.originalEvent;
        var s = e.data.scrollbar;
      ...
      $(t).find("select").change({"p":p, "g":g}, function(event) {
        ...
        var idx = $(this).attr("name").substring(spl + 1) * 1;
        event.data.p.data[idx][$(this).attr("name").substring(0, spl)] = $(this).val();
        $(this).closest("table").trigger("onSelectChanged", [event.data.p.module, $(this).attr("name").substring(0, spl), event.data.p.data[idx], event.data.g, event.data.p]);
      });
      $(t).find("button").on("click", {"p":p, "g":g}, function(event) {
        event.preventDefault();
        var spl = $(this).attr("name").lastIndexOf("_");
