...
var EMD = function($, window, document, undefined) {
  ...
  return{go:function(ctr, serverUrl) {
    ...
    OpenLayers.theme = EMD.context_root + "/css/theme/ygpa/";
  ...
  }, context_root:"", gis_engine_url:"http://lfitsvr.iptime.org:8080/G2DataService/GService?", serverUrl:"", vworldKey:"", window_seq:0, win_x:80, win_y:40, drag_handle:null, dragging:false, drag_ui:null, userinfo:null, popupCaller:null, popupId:null, popupParams:null, map:null, map_state:null, map_value:null, map_callback:null, gyhmap:null, wfs:null, lotarea:null, lotareaProtocol:null, panel:null, draw:null, edit:null, edit_win:null, del:null, info:null, save:null, saveStrategy:null, transaction:{type:null, 
  callbackWindowId:null}, map_panel:null, userLayer:{landCode:null, prtFclty:null, gisAssetsCd:null, portArea:null, quayGroup:null, quayCd:null, berthCd:null, constructCd:null, fcltyMarker:null}, userStyle:{assetCdStyle:{"default":null, "useSttus":null, "notUsgArea":null, "feeSttus":null, "reqSttus":null, "evlDtls":null}, quayStyle:{"default":null, "useSttus":null, "notUsgArea":null, "feeSttus":null, "reqSttus":null}}, popup:{prtFcltyInfo:null, lotAreaInfo:null}, protocols:{prtFclty:null, gisAssetsCd:null}, 
  selectControl:null, selectInfoControl:null, dragFeature:null, maxBounds:new OpenLayers.Bounds(-3E4, -6E4, 494288, 988576), maxRes:0.0439453125, maxZoom:14, module_instance:[], init:{frame_breaker:function() {
    ...
    EMD.map = new OpenLayers.Map("desktop", {div:"desktop", projection:new OpenLayers.Projection("EPSG:900913"), displayProjection:new OpenLayers.Projection("EPSG:5186"), units:"m", numZoomLevels:19, allOverlays:true, maxExtent:initExtent, theme:"css/theme/ygpa/style.css"});
    var basemap = new EMD.gis.OpenLayersVworld("\ubc30\uacbd\uc9c0\ub3c4", {isBaseLayer:true, maxExtent:initExtent, type:"png", numZoomLevels:19, projection:new OpenLayers.Projection("EPSG:900913")});
    ...
    EMD.userLayer.gisAssetsCd.events.register("featureadded", null, function(e) {
    });
    EMD.userLayer.gisAssetsCd.events.register("featureselected", null, function(e) {
      ...
      $.post(EMD.context_root + "/maps/assets/gamAssetCdInfo.do", [{name:"gisAssetsPrtAtCode", value:e.feature.attributes.GIS_ASSETS_PRT_AT_CODE}, {name:"gisAssetsCd", value:e.feature.attributes.GIS_ASSETS_CD}, {name:"gisAssetsSubCd", value:e.feature.attributes.GIS_ASSETS_SUB_CD}]).done(function(data) {
        ...
        $(popup.contentDiv).resize(function() {
          popup.updateSize();
        });
      });
    });
    ...
    EMD.userLayer.fcltyMarker = new OpenLayers.Layer.Vector("\uc2dc\uc124\ubb3c", {minScale:15E3});
    EMD.userLayer.fcltyMarker.events.register("featureselected", null, function(e) {
      ...
      {name:"prtFcltySe", value:e.feature.attributes.prtFcltySe}]}).done(function(data) {
        if (data.resultCode == 0) {
          ...
          EMD.popup.prtFcltyInfo = new OpenLayers.Popup.FramedCloud("\ud56d\ub9cc\uc2dc\uc124 \uc18d\uc131", new OpenLayers.LonLat(fclty.loCrdnt, fclty.laCrdnt), new OpenLayers.Size(250, 150), htmlInfo, null, true);
    ...
    EMD.selectControl = new OpenLayers.Control.SelectFeature([EMD.lotarea, EMD.userLayer.gisAssetsCd, EMD.userLayer.prtFclty, EMD.userLayer.fcltyMarker], {clickout:true, toggle:true, multiple:true, hover:false, multipleKey:"shiftKey"});
    ...
    EMD.userLayer.fcltyMarker.setVisibility(true);
    EMD.lotarea.setVisibility(true);
    EMD.map.addLayers([basemap, EMD.lotarea, EMD.userLayer.gisAssetsCd, EMD.userLayer.prtFclty, EMD.wfs, EMD.userLayer.fcltyMarker]);
    EMD.map.addControl(EMD.selectControl);
    EMD.selectControl.activate();
    var layerSwitcer = new OpenLayers.Control.LayerSwitcher;
    EMD.map.addControl(layerSwitcer);
    layerSwitcer.dataLbl.innerText = "\ub808\uc774\uc5b4 \ubaa9\ub85d";
    EMD.map.addControl(new OpenLayers.Control.MousePosition({prefix:"\uc9c0\ub3c4 \uc88c\ud45c : ", separator:" | ", numDigits:6, displayProjection:new OpenLayers.Projection("EPSG:5186")}));
    ...
    $.ajax({url:EMD.context_root + "/maps/fclty/gamFcltyList.do", type:"POST", dataType:"json", global:false, contentType:"application/x-www-form-urlencoded; charset=UTF-8", data:[]}).done(function(data) {
      if (data.resultCode == 0) {
        ...
        EMD.userLayer.fcltyMarker.redraw();
      }
    });
  }, ui_init:function() {
    $("#sideMenu").sidr("open", "sidr-main");
  }, user_init:function() {
    $.ajax({url:EMD.context_root + "/main/getUserInfo.do", type:"POST", dataType:"json", global:false, contentType:"application/x-www-form-urlencoded; charset=UTF-8", data:""}).done(function(data) {
    ...
    EMD.util.create_window("\uc790\uc0b0\uc784\ub300\uad00\ub9ac \ud604\ud669", EMD.context_root + "/asset/rent/gamAssetPopupInqire.do", null, {action:"loadStart"});
    ...
    var features = EMD.userLayer.fcltyMarker.getFeatureBy("_fcltyObjCode", prtFclty.gisAssetsPrtAtCode + ":" + prtFclty.gisAssetsCd + ":" + prtFclty.gisAssetsSubCd + ":" + prtFclty.prtFcltySe + ":" + prtFclty.gisPrtFcltyCd + ":" + prtFclty.gisPrtFcltySeq);
    EMD.userLayer.fcltyMarker.removeFeatures(features);
    EMD.gis._markerClick.deactivate();
    ...
    EMD.userLayer.fcltyMarker.addFeatures(marker);
  }, goLocation:function(lat, lon) {
    var pos = new OpenLayers.LonLat(lon, lat);
    EMD.map.setCenter(pos);
  }, selectPrtFclty:function(prtFclty) {
    var features = EMD.userLayer.fcltyMarker.getFeatureBy("_fcltyObjCode", prtFclty.gisAssetsPrtAtCode + ":" + prtFclty.gisAssetsCd + ":" + prtFclty.gisAssetsSubCd + ":" + prtFclty.prtFcltySe + ":" + prtFclty.gisPrtFcltyCd + ":" + prtFclty.gisPrtFcltySeq);
    EMD.selectInfoControl.select(features);
  }, selectAssetCdFeature:function(gisAssetsPrtAtCode, gisAssetsCd, gisAssetsSubCd, option) {
    var features = EMD.userLayer.fcltyMarker.getFeatureBy("_assetCdObjCode", prtFclty.gisAssetsPrtAtCode + ":" + prtFclty.gisAssetsCd + ":" + prtFclty.gisAssetsSubCd + ":" + prtFclty.prtFcltySe + ":" + prtFclty.gisPrtFcltyCd + ":" + prtFclty.gisPrtFcltySeq);
    EMD.selectInfoControl.select(features);
    var opts = option;
  ...
  }, removeFeature:function(layer, obj) {
    ...
    for (i = 0;i < len;i++) {
      ...
      if (feature && feature.attributes) {
        for (var j = 0;j < row.length;j++) {
          if (feature.attributes["GIS_ASSETS_PRT_AT_CODE"] === obj["gisAssetsPrtAtCode"] && (feature.attributes["GIS_ASSETS_CD"] === objr["gisAssetsCd"] && feature.attributes["GIS_ASSETS_SUB_CD"] === obj["gisAssetsSubCd"])) {
    ...
    for (i = 0;i < foundFeatures.length;i++) {
      ...
      feature.state = OpenLayers.State.DELETE;
      layer.events.triggerEvent("afterfeaturemodified", {feature:feature});
      feature.renderIntent = "select";
      ...
      if (feature && feature.attributes) {
        for (var j = 0;j < row.length;j++) {
          ...
          if (feature.attributes["GIS_ASSETS_PRT_AT_CODE"] === r["gisAssetsPrtAtCode"] && (feature.attributes["GIS_ASSETS_CD"] === r["gisAssetsCd"] && feature.attributes["GIS_ASSETS_SUB_CD"] === r["gisAssetsSubCd"])) {
            foundFeatures.push(feature);
            EMD.map.zoomToExtent(feature.bounds);
    ...
    if (foundFeatures.length == 0) {
      ...
      if (foundFeatures.length > 0) {
        var polygon = newfeature.geometry;
        foundFeatures[0].geometry = polygon.transform(layer.projection, EMD.map.getProjection());
    ...
    if (layer == EMD.userLayer.gisAssetsCd) {
      return feature.attributes["GIS_ASSETS_PRT_AT_CODE"] === obj["gisAssetsPrtAtCode"] && (feature.attributes["GIS_ASSETS_CD"] === obj["gisAssetsCd"] && feature.attributes["GIS_ASSETS_SUB_CD"] === obj["gisAssetsSubCd"]);
    }
    return false;
  ...
  }, OpenLayersVworld:OpenLayers.Class(OpenLayers.Layer.XYZ, {name:"vworldMap", url:["http://xdworld.vworld.kr:8080/2d/Base/201310/${z}/${x}/${y}.png"], sphericalMercator:false, transitionEffect:"resize", buffer:1, displayOutsideMaxExtent:true, initialize:function(name, options) {
  ...
  initialize:function(name, options) {
    ...
    switch(role) {
      case "loadMap":
        ...
        if (src == null) {
          EMD.gis.loadMapDataCode(module, button.data("gis-layer"), button.data("gis-style"), button.data("code-id"), button.data("value"));
        } else {
      ...
      case "showMap":
        var src = button.data("flexi-grid");
        if (src == null) {
          EMD.gis.showMapDataCode(module, button.data("gis-layer"), button.data("code-id"), button.data("value"));
        } else {
      ...
      case "addFearutre":
        var src = button.data("flexi-grid");
        if (src == null) {
          EMD.gis.addFeatureCode(module, button.data("gis-layer"), button.data("code-id"), button.data("value"));
        } else {
...
EmdModule.prototype.addAssetsCodeByLotcode = function(featureid, assetCode) {
  ...
  feature.attributes = {GIS_ASSETS_PRT_AT_CODE:assetCode.gisAssetsPrtAtCode, GIS_ASSETS_CD:assetCode.gisAssetsCd, GIS_ASSETS_SUB_CD:assetCode.gisAssetsSubCd, GIS_ASSETS_NM:assetCode.gisAssetsNm, GIS_ASSETS_LOCPLC:assetCode.gisAssetsLocplc, GIS_ASSETS_LNM:assetCode.gisAssetsLnm, GIS_ASSETS_LNM_SUB:assetCode.gisAssetsLnmSub};
  EMD.userLayer.gisAssetsCd.addFeatures([feature]);
  EMD.userLayer.gisAssetsCd.redraw();
};
