...
var EMD = function($, window, document, undefined) {
  ...
  callbackWindowId:null}, map_panel:null, userLayer:{landCode:null, prtFclty:null, gisAssetsCd:null, fcltyMarker:null}, popup:{prtFcltyInfo:null, lotAreaInfo:null}, protocols:{prtFclty:null, gisAssetsCd:null}, selectControl:null, selectInfoControl:null, dragFeature:null, maxBounds:new OpenLayers.Bounds(-3E4, -6E4, 494288, 988576), maxRes:0.0439453125, maxZoom:14, module_instance:[], init:{frame_breaker:function() {
    ...
    EMD.userLayer.gisAssetsCd = new OpenLayers.Layer.Vector("\uc790\uc0b0\uc815\ubcf4", {minScale:9E3, strategies:[new OpenLayers.Strategy.Fixed], projection:new OpenLayers.Projection("EPSG:5186"), protocol:EMD.protocols.gisAssetsCd, styleMap:new OpenLayers.StyleMap({"default":{strokeColor:"#000000", strokeOpacity:1, strokeWidth:1, fillColor:"#303294", fillOpacity:0.5, pointRadius:6, pointerEvents:"visiblePainted", label:"\uc2dc\uc124\ucf54\ub4dc: ${GIS_ASSETS_CD}-${GIS_ASSETS_SUB_CD}\n${GIS_ASSETS_NM}", 
  ...
  initialize:function(name, options) {
    ...
    $("#" + win_id).find("input.ygpaCmmnCl").each(function() {
      ...
      if (default_prompt != "") {
        if ($(input_item).data("value") === default_value || $(this.module).val() === default_value) {
          $("#" + win_id).find("#" + input_id + "_select").append('<option value="' + default_value + '" selected>' + default_prompt + "</option>");
        } else {
    ...
    $("#" + win_id).find(":text").on("keyup", {module:$("#" + win_id)[0].module}, function(e) {
      var code = e.which;
      if (code == 13) {
        e.preventDefault();
        if (e.data.module.onSubmit != undefined) {
          e.data.module.onSubmit();
        }
      }
    });
  }}};
...
EmdModule.prototype.makeMultiDivValues = function(selector, list, func) {
  ...
  for (var i = 0;i < list.length;i++) {
    var row = selobj.children(":eq(" + i + ")");
    row[0].func = func;
    row[0].func(list[i]);
    this.makeDivValues(row, list[i], func);
  }
  if (list.length == 0) {
