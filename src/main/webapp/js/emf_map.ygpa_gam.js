...
//			    					    	EMD.util.create_window('자산정보추가', '/code/assets/gamAssetCodeMngt.do', null, {action: "addLotcodeFeature", feature: resp.reqFeatures});
			        			    	}
			        			    });
//							      EMD.saveStrategy.save();
...
//                var feature = e.feature;
			...
			EMD.userLayer.locArea.events.register('featureunselected', null, function(e) {
		 	...
		 	EMD.userLayer.gisAssetsCd = new OpenLayers.Layer.Vector("자산정보", {
				...
				styleMap: new OpenLayers.StyleMap(EMD.userStyle.assetCdStyle['default'], {extendDefault: false})
//				renderers: renderer
		 	});
			...
			EMD.userLayer.gisAssetsCd.events.register('featureselected', null, function(e) {
				EMD.gis.openPopup(e.feature, "/maps/assets/gamAssetCdInfo.do", [
				       				       { name: 'gisAssetsPrtAtCode', value:e.feature.attributes.PRT_AT_CD },
				    				       { name: 'gisAssetsCd', value:e.feature.attributes.PRT_CD },
				    				       { name: 'gisAssetsSubCd', value:e.feature.attributes.PRT_SUB_CD },
	                ...
	                $("button[data-role='modifyFeature']", popup.contentDiv).button({icons: {
						  	primary: 'ui-icon-wrench'}}).click({feature: f}, function(event) {
		                    	EMD.util.create_window($(this).text(), "/code/assets/gamAssetCodeMngt.do", null, {
		                    		action: "setFeature"
	                    			,gisPrtAtCode: $(this).data("assets-prt-at-code")
	                    			,gisAssetsCd: $(this).data("assets-cd")
	                    			,gisAssetsSubCd: $(this).data("assets-sub-cd")
	                    			,bjdCode: $(this).data("bjd-code")
	                    			,addr: $(this).data("addr")
	                    			,lnm: $(this).data("lnm")
	                    			,lnmSub: $(this).data("lnm-sub")
	                    			,feature: event.data.feature
                			});
						  });
	                $("button[data-role='removeFeature']", popup.contentDiv).button({icons: {
						  	primary: 'ui-icon-trash'}}).click({feature: f}, function(event) {
...
//			                    EMD.protocol.gisAssetsCd.commit([event.data.feature]);
						  });
	                popup.updateSize();
				});
            });
			...
			EMD.userStyle.assetsRentDetailStyle['default'] = {
					'default':{
						strokeColor: "#000000",
						strokeOpacity: 1,
						strokeWidth: 1,
...
//						label : "사용면적: ${USAGE_AR}\n${USAGE_PD_FROM}~${USAGE_PD_TO}",
			...
			EMD.strategies.assetsRent = new OpenLayers.Strategy.Refresh();
		 	...
		 	EMD.userLayer.assetsRent = new OpenLayers.Layer.Vector("임대정보", {
		 		minScale: 10000,
				strategies: [EMD.strategies.assetsRent, EMD.saveStrategy],
		        projection: new OpenLayers.Projection("EPSG:5186"),
...
//				renderers: renderer
			...
			EMD.userLayer.assetsRent.events.register('featuresadded', null, function(e) {
			});
...
//			EMD.strategies.assetsRentDetail = new OpenLayers.Strategy.Filter();
//			EMD.strategies.assetsRentDetail.deactivate();

		 	EMD.userLayer.assetsRentDetail = new OpenLayers.Layer.Vector("시설임대상세", {
		 		minScale: 20000,
				strategies: [EMD.strategies.assetsRent, EMD.saveStrategy],
		        projection: new OpenLayers.Projection("EPSG:5186"),
				protocol: EMD.protocols.assetsRentDetail,
		 		displayInLayerSwitcher: false,	// if debug
				styleMap: new OpenLayers.StyleMap(EMD.userStyle.assetsRentDetailStyle['default'])
//				renderers: renderer
		 	});

			EMD.userLayer.assetsRentDetail.events.register('featuresadded', null, function(e) {
			});

			EMD.userLayer.assetsRentDetail.events.register('featureselected', null, function(e) {
				EMD.gis.openPopup(e.feature
						, "/maps/assets/gamMNGInfo.do"
						, [
			...
			EMD.userLayer.assetsRentDetail.events.register('featureunselected', null, function(e) {
                var feature = e.feature;
                if(feature.popup!=null) {
                    EMD.map.removePopup(feature.popup);
                    feature.popup.destroy();
	...
	//				contentType: "text/plain; charset=UTF-8",
			...
			if($DEBUG) {
			 	EMD.selectControl = new OpenLayers.Control.SelectFeature(
						[EMD.lotarea, EMD.userLayer.gisAssetsCd, EMD.userLayer.locArea, EMD.userLayer.assetStats, EMD.userLayer.prtFclty, EMD.userLayer.fcltyConstMarker, EMD.userLayer.fcltySocMarker, EMD.userLayer.fcltyMechMarker, EMD.userLayer.fcltyInfoTeleMarker, EMD.userLayer.fcltyElecPowerMarker],
						{
...
//						    toggleKey: "ctrlKey", // ctrl key removes from selection
			 	...
			 	EMD.selectControl = new OpenLayers.Control.SelectFeature(
						[EMD.userLayer.landCode, EMD.userLayer.gisAssetsCd, EMD.userLayer.locArea, EMD.userLayer.assetStats, EMD.userLayer.fcltyConstMarker, EMD.userLayer.fcltySocMarker, EMD.userLayer.fcltyMechMarker, EMD.userLayer.fcltyInfoTeleMarker, EMD.userLayer.fcltyElecPowerMarker],
						{
						    clickout: true, toggle: true,
						    multiple: true, hover: false,
//						    toggleKey: "ctrlKey", // ctrl key removes from selection
...
/*	        	EMD.strategies.assetsRent.setFilter(new OpenLayers.Filter.Logical({
...
//            });
    ...
    gis: {
    	 ...
    	 * @param feature - 피처 오브젝트
    	 * @param url - 팝업 내용을 로드할 URL
    	 * @param params - 팝업 내용을 로드할때 전송할 파라미터
    	 * @param func - 팝업이 완료 되고 실행 할 함수 (파라미터로 팝업 객체를 popup 에 담아 feature 가 전송 된다.)
    	 * @returns - 없음
    	 */
    	openPopup: function(feature, url, params, func) {
			$.ajax({
				...
				success: function(data) {
		                ...
		                EMD.map.addPopup(popup);
		                EMD.gis.makePopup(popup, this.feature);
    	 ...
    	 * @param layerName - 피처를 추가할 레이어 명
    	 * @param pnu - 조회 할 지번 코드
    	 * @param retfunc - 호출을 완료 한 후 호출 할 함수 로딩 된 feature를 변수로 전송한다.
    	 * @returns - 없음
    	 */
...
//        						      var bounds=polygon.getBounds();
        			        ...
        			        var v = new OpenLayers.Feature();
        			        v.geometry = feature.geometry.clone();
        			        v.attributes = {PRT_AT_CD:assetsCd.gisAssetsPrtAtCode, PRT_CD:assetsCd.gisAssetsCd, PRT_SUB_CD:assetsCd.gisAssetsSubCd, PRT_NM:assetsCd.gisAssetsNm, LOCPLC:assetsCd.gisAssetsLocplc, GIS_ASSETS_LNM:assetsCd.gisAssetsLnm, GIS_ASSETS_LNM_SUB:assetsCd.gisAssetsLnmSub};
...
//  				module: input_item,
  			...
  			showStatusMapView:function(module, mapLayer, grid, mapStyle, value, labelField, popup_url, popup_func, getUserColor, getUserLabel) {
  				var g = module.$('#'+grid);
  				...
  				if(popup_url!=undefined) {
  					purl=popup_url;
  					if(typeof popup_func =="string") pf=module[popup_func];
  					else pf=popup_func;
  				}
				len = layer.features.length;
...
//				target.addFeatures(layer.features);
				...
				default:
					EMD.gis.getStatsColor = function(feature) {
						var b=Math.round(feature.attributes['value']/feature.attributes['max-value']*100);
						var hue = Math.floor((b) * 120 / 100);  // go from green to red
						//var sat = Math.abs(b - 50)/50;   // fade to white as it approaches 50
						return EMD.util.hsv2rgb(hue, 1, 1);
					};
					EMD.gis.getStatsLabel = function(feature) {
						if(feature.attributes['label']==undefined) return "";
				...
				target.events.register('featureselected', {pf:pf, purl:purl}, function(e) {
					if(this.purl!=null) {
						EMD.gis.openPopup(e.feature, this.purl, EMD.util.objectToArray(e.feature.attributes), this.pf);
					}
					else {
						EMD.gis.openPopup(e.feature, "/maps/assets/gamAssetCdInfo.do", EMD.util.objectToArray(e.feature.attributes), this.pf);
					}
...
//				EMD.gis.selectFeatureData(module, layerName, row, true, true);
				...
				while(done) {
					...
					if(foundFeatures.length==0) {
						if(layerName=="assetsRent") {
							$.ajax({
				  				url: EMD.context_root+"/maps/selectMapsAssetRentList.do",
				  				type: 'POST',
				  				module: module,
				  				dataType: 'json',
				  				contentType: "application/x-www-form-urlencoded; charset=UTF-8",
								...
								success: function (result) {
										EMD.gis._showFeatureData(module, "assetsRentDetail", result.resultList);
								}
							});
							return;
						}
						else if(layerName=="assetsRentDetail") {
							layerName="gisAssetsCd";
...
//		    			allowDepress: true
		    		...
		    		EMD.selectControl.deactivate();		// 객체 정보를 표시 하지 않도록 변경한다.
...
//		    			allowDepress: true
		...
		setFeatureCode: function(module, layerName, value, feature) {
			...
			feature.geometry = polygon.transform(
					    	 EMD.map.getProjection(), new OpenLayers.Projection("EPSG:5186")
		          );
			feature.state=OpenLayers.State.UPDATE;
	        EMD.protocols.gisAssetsCd.update(feature);
	        EMD.protocols.gisAssetsCd.commit([feature], {
		    	callback:function(resp) {
    			    if(resp.error) {
    			        //console.log('error');
    			        return -1;
    			    ...
    			    EMD.userLayer.gisAssetsCd.removeFeatures(resp.reqFeatures);
			    	EMD.userLayer.gisAssetsCd.addFeatures(resp.reqFeatures);
			    	EMD.userLayer.gisAssetsCd.redraw();
		    	}
	        });
...
//	    			        EMD.gis.storeLayerObject(EMD.userLayer.assetsRentDetail, resp.data.f, resp.data.obj);
...
//	    				EMD.gis.modifyAssetCdFeature(EMD.draw._mode.param, EMD.edit.feature);
  			...
  			checkLayerObject: function(layer, obj) {
  				if(layer==EMD.userLayer.gisAssetsCd) {
  					var prtAtCode=obj['gisAssetsPrtAtCode']==undefined?obj['PRT_AT_CD']:obj['gisAssetsPrtAtCode'];
  					var gisAssetsCd=obj['gisAssetsCd']==undefined?obj['PRT_CD']:obj['gisAssetsCd'];
  					var gisAssetsSubCd=obj['gisAssetsSubCd']==undefined?obj['PRT_SUB_CD']:obj['gisAssetsSubCd'];
  					...
  					var prtAtCode=obj['prtAtCode']==undefined?obj['PRT_AT_CD']:obj['prtAtCode'];
  			...
  			compareLayerObject: function(layer, feature, obj) {
  				if(layer==EMD.userLayer.gisAssetsCd) {
					...
					&& feature.attributes['PRT_CD'] === gisAssetsCd
					&& feature.attributes['PRT_SUB_CD'] === gisAssetsSubCd;
  				}
  				else if(layer==EMD.userLayer.assetRentStats) {
  					var prtAtCode=obj['prtAtCode']==undefined?obj['PRT_AT_CD']:obj['prtAtCode'];
  					var mngYear=obj['mngYear']==undefined?obj['MNG_YEAR']:obj['mngYear'];
  			...
  			storeLayerObject:function(layer, feature, obj) {
  				if(layer==EMD.userLayer.gisAssetsCd) {
  					feature.attributes['PRT_AT_CD'] = obj['gisAssetsPrtAtCode'];
					feature.attributes['PRT_CD'] = obj['gisAssetsCd'];
					feature.attributes['PRT_SUB_CD'] = obj['gisAssetsSubCd'];
					feature.attributes['PRT_NM'] = obj['gisAssetsNm'];
					if(feature.state!=OpenLayers.State.INSERT) feature.state=OpenLayers.State.UPDATE;
  				}
  				else if(layer==EMD.userLayer.assetsRent) {
  					feature.attributes['PRT_AT_CD'] = obj['prtAtCode'];
					...
					if(feature.state!=OpenLayers.State.INSERT) feature.state=OpenLayers.State.UPDATE;
  					...
  					feature.attributes['MNG_NO']=obj['mngNo'];
  					feature.attributes['MNG_CNT']=obj['mngCnt'];
  					feature.attributes['MNG_SEQ']=obj['assetsUsageSeq'];
  					feature.attributes['GIS_PRT_CD']=obj['gisAssetsPrtAtCode'];
  					feature.attributes['GIS_CD']=obj['gisAssetsCd'];
  					feature.attributes['GIS_SUB_CD']=obj['gisAssetsSubCd'];
...
//    				e.feature.attributes.PRT_CD=param.gisAssetsCd;
//    				e.feature.attributes.PRT_SUB_CD=param.gisAssetsSubCd;
//    				e.feature.attributes.PRT_NM=param.gisAssetsNm;
...
//            });
    ...
    util: {
    	 ...
    	 * @param d - 입력 날짜, 지정 안하면 현재 날짜에 대한 시간 도장을 리턴한다.
    	 * @returns
    	 */
		...
		/**
		 * 메세지 팝업을 출력한다.
		 * @param obj - 출력할 대상 객체 (INPUT, BUTTON 타입 속성)
		 * @param msg - 출력할 메세지
		 * @returns
		 */
      ...
      /**
       * 윈도우 창을 생성한다.
       * @param win_title - 창 제목 (String)
       * @param win_url - 호출 창 컨텐츠 주소 (String)
       * @param argv - 호출할때 서버에 전달 할 파라메터 (Object)
       * @param init_params - 창이 열리고 loadComplete  할때 전달할 변수 (Object)
       * @param parent - 부모창 모듈
       * @returns - 윈도우 id
       */
...
//    		  win.style.visibility='hidden';
		  ...
		  win.innerHTML='<div class="abs window_inner">'+
			...
			'</div><div class="abs window_content"></div></div><div class="abs window_bottom ">여수광양항만공사 지도프레임워크 ver : 2015.01.09.001</div><span class="abs ui-resizable-handle ui-resizable-se"></span>';
...
//		};);
      ...
      _clickButtonExec: function(module, button, button_id) {
		  ...
		  case 'loadStatsMap':	// 통계 맵 조회
			  var src=button.data('flexi-grid');
			  if(src!=null) {
				  EMD.gis.showStatusMapView(module, button.data('gis-layer'), src, button.data('map-style'), button.data('value'), button.data('label-field'), button.data('popup-url'), button.data('popup-function'));
			  }
			  break;
 ...
 */
EmdModule.prototype.makeFormArgs = function(selector, type) {
...
/*	var feature = EMD.lotarea.getFeatureById(featureid);
	feature.attributes = {
			PRT_AT_CD: assetCode.gisAssetsPrtAtCode,
			PRT_CD: assetCode.gisAssetsCd,
			PRT_SUB_CD: assetCode.gisAssetsSubCd,
			PRT_NM: assetCode.gisAssetsNm,
			LOCPLC: assetCode.gisAssetsLocplc,
	};
	EMD.userLayer.gisAssetsCd.addFeatures([feature]);
	EMD.userLayer.gisAssetsCd.redraw();*/
};
...
EmdPopupModule.prototype.makeFormArgs = function(selector, type) {
	var obj_type=type||'array';
	var args = [];
	this.$(selector).find(':input').not('.frmwrkAuto').not(".skipValue").each( function() {
		var value='';
