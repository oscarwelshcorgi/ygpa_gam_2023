...
//#}
	...
	_postRowProcessing:function(r,donly){
		...
		for (var i = 0; i < r.childNodes.length; i++){
			...
			//if (s)
				//c.align=s; // remark by eunsungj replace next line
			if (s)
				c.style.cssText+=";text-align: "+s;
...
function eXcell_co(cell){
	...
	this.edit=function(){
		...
		this.list.onclick=function(e){
			...
			editor_obj.cell.combo_value = editor_obj.list.value;
	...
	this.detach=function(){
		if (this.val != this.getValue()){
			this.grid.setRowAttribute($(this.cell).parent("tr")[0].idd, this.grid.getColumnId(this.cell._cellIndex), this.getValue());
			this.val=this.getValue();
			this.cell.wasChanged=true;
		}
